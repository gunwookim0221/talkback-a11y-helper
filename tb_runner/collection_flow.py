...
def _sync_local_tab_logic_dependencies() -> None:
    local_tab_logic.LOCAL_TAB_ACTIVE_TTL_STEPS = LOCAL_TAB_ACTIVE_TTL_STEPS
    local_tab_logic._TRANSITION_FAST_ACTION_WAIT_SECONDS = _TRANSITION_FAST_ACTION_WAIT_SECONDS
    local_tab_logic.COLLECTION_FLOW_SCROLL_DECISION_DEBUG_VERSION = COLLECTION_FLOW_SCROLL_DECISION_DEBUG_VERSION
    local_tab_logic.COLLECTION_FLOW_SCROLL_READY_VERSION = COLLECTION_FLOW_SCROLL_READY_VERSION
    local_tab_logic.log = log
    local_tab_logic.parse_bounds_str = parse_bounds_str
    local_tab_logic.normalize_move_result = normalize_move_result
    local_tab_logic.build_row_semantic_fingerprint = build_row_semantic_fingerprint
    local_tab_logic._truncate_debug_text = _truncate_debug_text
    local_tab_logic._normalize_logical_text = _normalize_logical_text
    local_tab_logic._summarize_candidate_labels = _summarize_candidate_labels
    local_tab_logic._representative_focus_matches = _representative_focus_matches
    local_tab_logic._extract_cta_grace_focus_meta = _extract_cta_grace_focus_meta
    local_tab_logic._is_low_value_leaf_text = _is_low_value_leaf_text
    local_tab_logic._clear_active_container_group = _clear_active_container_group
    local_tab_logic._build_row_object_signature = _build_row_object_signature
    local_tab_logic._collect_step_candidate_priority_groups = _collect_step_candidate_priority_groups
    local_tab_logic._filter_content_candidates_for_phase = _filter_content_candidates_for_phase
    local_tab_logic._apply_spatial_priority_to_candidates = _apply_spatial_priority_to_candidates
    local_tab_logic._candidate_cluster_logical_signature = _candidate_cluster_logical_signature
    local_tab_logic._candidate_cluster_signature = _candidate_cluster_signature
    local_tab_logic._candidate_logical_signature = _candidate_logical_signature
    local_tab_logic._candidate_object_signature = _candidate_object_signature
    local_tab_logic._cluster_candidate_sort_key = _cluster_candidate_sort_key
    local_tab_logic._cluster_display_name = _cluster_display_name
    local_tab_logic._filter_realign_target_candidates = _filter_realign_target_candidates
    local_tab_logic._focus_anchor_match_reason = _focus_anchor_match_reason
    local_tab_logic._maybe_realign_focus_to_representative = _maybe_realign_focus_to_representative
    local_tab_logic._current_local_tab_phase_label = _current_local_tab_phase_label
    local_tab_logic._extract_cta_node_label = _extract_cta_node_label
    local_tab_logic._node_label_blob = _node_label_blob
    local_tab_logic._apply_cta_node_to_row = _apply_cta_node_to_row
...
class MainLoopState:
    ...
    forced_local_tab_target_rid: str
    ...
    current_local_tab_content_entry_probe_attempted: bool
    active_container_group_signature: str
    active_container_group_remaining: set[str]
    active_container_group_labels: dict[str, str]
    completed_container_groups: set[str]
    last_selected_local_tab_signature: str
...
def _select_better_cluster_representative(
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._select_better_cluster_representative(
...
def _build_candidate_object_signature(*, rid: str, bounds: str, label: str) -> str:
    return container_group_logic._build_candidate_object_signature(rid=rid, bounds=bounds, label=label)
...
def _clear_active_container_group(state: Any, *, reason: str) -> None:
    active_signature = str(getattr(state, "active_container_group_signature", "") or "").strip()
    remaining = set(getattr(state, "active_container_group_remaining", set()) or set())
    if remaining:
        log(f"[STEP][container_group_end] reason='{reason}'")
    if reason == "consumed" and active_signature:
        completed = set(getattr(state, "completed_container_groups", set()) or set())
        completed.add(active_signature)
        state.completed_container_groups = completed
    state.active_container_group_signature = ""
    state.active_container_group_remaining = set()
...
def _clear_last_selected_local_tab_hint(state: Any, *, reason: str) -> None:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._clear_last_selected_local_tab_hint(state, reason=reason)
...
def _write_last_selected_local_tab_hint(
    ...
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._write_last_selected_local_tab_hint(state, signature=signature, rid=rid, label=label, bounds=bounds, reason=reason)


def _candidate_object_signature(candidate: dict[str, Any]) -> str:
    return container_group_logic._candidate_object_signature(candidate)


def _candidate_cluster_signature(candidate: dict[str, Any]) -> str:
    return container_group_logic._candidate_cluster_signature(candidate)
...
def _normalize_logical_text(value: str) -> str:
    return container_group_logic._normalize_logical_text(value)
...
def _candidate_cluster_logical_signature(candidate: dict[str, Any]) -> str:
    return container_group_logic._candidate_cluster_logical_signature(candidate)


def _candidate_container_group_visual_order_key(candidate: dict[str, Any]) -> tuple[int, int]:
    return container_group_logic._candidate_container_group_visual_order_key(candidate)


def _sort_active_container_group_candidates(
    ...
    *,
    ...
    if not str(getattr(state, "active_container_group_signature", "") or "").strip():
        return list(candidates)
    remaining = set(getattr(state, "active_container_group_remaining", set()) or set())
    if not remaining:
        return list(candidates)
    group_candidates = [candidate for candidate in candidates if _candidate_object_signature(candidate) in remaining]
    if len(group_candidates) <= 1:
        return list(candidates)
    if any(not parse_bounds_str(str(candidate.get("bounds", "") or "").strip()) for candidate in group_candidates):
        return list(candidates)
    ordered_group = sorted(group_candidates, key=_candidate_container_group_visual_order_key)
    group_ids = {id(candidate) for candidate in group_candidates}
    group_iter = iter(ordered_group)
    ordered = [next(group_iter) if id(candidate) in group_ids else candidate for candidate in candidates]
    log(
        f"[STEP][container_group_visual_order] "
        f"remaining='{_truncate_debug_text(_summarize_candidate_labels(group_candidates), 120)}' "
        f"ordered='{_truncate_debug_text(_summarize_candidate_labels(ordered_group), 120)}'"
    )
    return ordered
...
def _filter_content_candidates_for_phase(
    ...
    *,
    ...
    completed_container_candidates: list[dict[str, Any]] = []
    candidate_group_signature = _container_group_signature(top_priority_candidates)
    if candidate_group_signature and candidate_group_signature in completed_groups:
        completed_container_candidates = list(top_priority_candidates)
        completed_ids = {id(candidate) for candidate in completed_container_candidates}
        selection_candidates = [candidate for candidate in selection_candidates if id(candidate) not in completed_ids]
        top_priority_candidates = []
        for candidate in completed_container_candidates[:5]:
            log(
                f"[STEP][container_group_skip] label='{_truncate_debug_text(str(candidate.get('label', '') or candidate.get('rid', '') or ''), 96)}' "
                "reason='group_already_consumed'"
            )
    active_remaining = set(getattr(state, "active_container_group_remaining", set()) or set())
    active_priority_candidates = [
        candidate for candidate in top_priority_candidates if _candidate_object_signature(candidate) in active_remaining
    ]
    if active_remaining and active_priority_candidates:
        top_priority_candidates = _sort_active_container_group_candidates(active_priority_candidates, state=state)
        container_priority_applied, container_priority_reason = True, "active_group"
    elif active_remaining and not active_priority_candidates:
        _clear_active_container_group(state, reason="signature_mismatch")
        container_priority_applied, container_priority_reason = _should_apply_container_priority_narrowing(
            top_priority_candidates,
            selection_candidates,
        )
    else:
    ...
    if container_priority_applied and container_priority_reason == "repeated_container_group":
        group_signatures = {_candidate_object_signature(candidate) for candidate in top_priority_candidates}
        candidate_group_signature = _container_group_signature(top_priority_candidates)
        if candidate_group_signature and candidate_group_signature in completed_groups:
            container_priority_applied = False
            container_priority_reason = "group_already_consumed"
            log("[STEP][container_group_start_skip] reason='already_consumed'")
        else:
            state.active_container_group_signature = candidate_group_signature or (str(getattr(state, "current_local_tab_signature", "") or "content").strip() or "content")
            state.active_container_group_remaining = set(group_signatures)
            state.active_container_group_labels = {
                _candidate_object_signature(candidate): str(candidate.get("label", "") or candidate.get("rid", "") or "").strip()
                for candidate in top_priority_candidates
            }
            top_priority_candidates = _sort_active_container_group_candidates(top_priority_candidates, state=state)
            log(
                f"[STEP][container_group_start] "
                f"candidates='{_truncate_debug_text(_summarize_candidate_labels(top_priority_candidates), 120)}'"
            )
    if container_priority_applied:
        selection_candidates = top_priority_candidates
        log(
            f"[STEP][container_priority_applied] "
            f"candidates='{_truncate_debug_text(_summarize_candidate_labels(top_priority_candidates), 120)}' "
            f"reason='{container_priority_reason}'"
        )
    elif top_priority_candidates:
        log(
            f"[STEP][container_priority_skip] "
            f"candidates='{_truncate_debug_text(_summarize_candidate_labels(top_priority_candidates), 120)}' "
            f"reason='{container_priority_reason}'"
        )
    ...
    return {
        ...
        "container_priority_applied": container_priority_applied,
        "container_priority_reason": container_priority_reason,
    }
...
def _should_apply_container_priority_narrowing(
    ...
    return container_group_logic._should_apply_container_priority_narrowing(
...
def _container_group_signature(container_candidates: list[dict[str, Any]]) -> str:
    return container_group_logic._container_group_signature(container_candidates)
...
def _build_local_tab_strip_signature(tab_candidates: list[dict[str, Any]]) -> str:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._build_local_tab_strip_signature(tab_candidates)
...
def _select_active_local_tab_candidate(
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._select_active_local_tab_candidate(tab_candidates=tab_candidates, row=row)


def _sort_local_tab_candidates_left_to_right(tab_candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._sort_local_tab_candidates_left_to_right(tab_candidates)


def _local_tab_state_display(*, rid: str = "", label: str = "") -> str:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._local_tab_state_display(rid=rid, label=label)
...
def _local_tab_candidate_matches_identity(
    ...
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._local_tab_candidate_matches_identity(candidate, rid=rid, label=label, bounds=bounds)
...
def _resolve_active_local_tab_candidate_for_progression(
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._resolve_active_local_tab_candidate_for_progression(state=state, sorted_tab_candidates=sorted_tab_candidates, row=row, previous_row=previous_row)
...
def _recover_local_tab_state_from_bottom_strip(
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._recover_local_tab_state_from_bottom_strip(state=state, row=row, previous_row=previous_row, bottom_strip_candidates=bottom_strip_candidates, reason=reason)
...
def _row_matches_pending_local_tab(
    ...
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._row_matches_pending_local_tab(row, pending_rid=pending_rid, pending_label=pending_label, pending_bounds=pending_bounds)


def _maybe_commit_pending_local_tab_progression(state: MainLoopState, row: dict[str, Any]) -> None:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._maybe_commit_pending_local_tab_progression(state, row)
...
def _record_pending_local_tab_progression(
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._record_pending_local_tab_progression(state=state, signature=signature, next_candidate=next_candidate, reason=reason)


def _format_bounds_for_log(bounds: tuple[int, int, int, int] | None) -> str:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._format_bounds_for_log(bounds)


def _extract_local_tab_candidate_bounds(candidate: Any) -> tuple[int, int, int, int] | None:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._extract_local_tab_candidate_bounds(candidate)


def _parse_local_tab_bounds_value(value: Any) -> tuple[int, int, int, int] | None:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._parse_local_tab_bounds_value(value)


def _collect_local_tab_viewport_bounds(client: A11yAdbClient, dev: str, target_bounds: tuple[int, int, int, int]) -> tuple[int, int]:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._collect_local_tab_viewport_bounds(client, dev, target_bounds)
...
def _tap_local_tab_bounds_center(
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._tap_local_tab_bounds_center(client=client, dev=dev, target=target, target_bounds=target_bounds)


def _clear_forced_local_tab_navigation(state: MainLoopState, *, reason: str) -> None:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._clear_forced_local_tab_navigation(state, reason=reason)


def _forced_local_tab_target_matches_row(state: MainLoopState, row: dict[str, Any]) -> bool:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._forced_local_tab_target_matches_row(state, row)
...
def _reset_content_phase_after_tab_switch(
    ...
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._reset_content_phase_after_tab_switch(state, active_label=active_label, active_rid=active_rid, active_signature=active_signature, active_bounds=active_bounds)


def _commit_forced_local_tab_target_success(state: MainLoopState) -> None:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._commit_forced_local_tab_target_success(state)
...
def _activate_forced_local_tab_target(
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._activate_forced_local_tab_target(client=client, dev=dev, state=state, step_idx=step_idx, wait_seconds=wait_seconds, announcement_wait_seconds=announcement_wait_seconds, announcement_idle_wait_seconds=announcement_idle_wait_seconds, announcement_max_extra_wait_seconds=announcement_max_extra_wait_seconds)


def _is_current_focus_on_local_tab_strip(state: MainLoopState, row: dict[str, Any]) -> bool:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._is_current_focus_on_local_tab_strip(state, row)
...
def _collect_step_candidate_priority_groups(
    ...
    *,
    ...
    if viewport_right - viewport_left >= 800 and viewport_bottom - viewport_top < 1200:
        viewport_top = min(viewport_top, 0)
        viewport_bottom = max(viewport_bottom, 1920)
    ...
    for node in visible_nodes:
        ...
        if _is_chrome_like_candidate(
            ...
            continue
        clickable_container_promoted = bool(
            actionable
            and height >= 100
            and (
                "linearlayout" in class_name
                or "viewgroup" in class_name
                or ("layout" in class_name and width_ratio >= 0.25)
            )
            and not passive_status
            and center_y > top_header_band
            and center_y < bottom_band_top
        )
        top_priority_container = bool(
            bool(node.get("clickable"))
            and bool(node.get("focusable") or node.get("effectiveClickable"))
            and height >= 100
            and (
                "linearlayout" in class_name
                or "viewgroup" in class_name
        ...
        representative = bool(
            ...
            or clickable_container_promoted
        ...
        if passive_status:
            score -= 550000
        if clickable_container_promoted:
            score += 220000
            container_promoted_candidates.append(label)
        if top_priority_container:
            score += 900000
            top_priority_container_candidates.append(label)
        ...
        candidate = {
            ...
            "clickable_container_promoted": bool(clickable_container_promoted),
...
def _is_row_persistent_bottom_strip_candidate(row: dict[str, Any]) -> bool:
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._is_row_persistent_bottom_strip_candidate(row)
...
def _maybe_reprioritize_persistent_bottom_strip_row(
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._maybe_reprioritize_persistent_bottom_strip_row(row=row, client=client, dev=dev, tab_cfg=tab_cfg, state=state, step_idx=step_idx, scenario_perf=scenario_perf)
...
def _maybe_select_next_local_tab(
    *,
    ...
    _sync_local_tab_logic_dependencies()
    return local_tab_logic._maybe_select_next_local_tab(client=client, dev=dev, state=state, row=row, scenario_id=scenario_id, step_idx=step_idx)
...
def _main_loop_phase(
    ...
    for step_idx in range(start_step_index, tab_cfg["max_steps"] + 1):
        ...
        content_phase_grace_remaining = int(getattr(state, "content_phase_grace_steps", 0) or 0)
        if content_phase_grace_remaining > 0:
            row["content_phase_grace_active"] = True
            log(
                f"[STEP][content_phase_grace_block] reason='within_grace_window' "
                f"remaining={content_phase_grace_remaining}"
            )
            state.content_phase_grace_steps = max(0, content_phase_grace_remaining - 1)
            if stop:
                stop = False
                reason = ""
...
def _maybe_recover_food_scan_screen(
    ...
    *,
    ...
    state.current_local_tab_content_entry_probe_attempted = False
