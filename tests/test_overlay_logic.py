...
def test_expand_overlay_marks_synthetic_first_row_with_protection_flag(monkeypatch):
    ...
    assert overlay_rows[0]["_is_overlay_first_row"] is True


def test_expand_overlay_excludes_first_row_fingerprint_from_duplicate_streak(monkeypatch):
    post_click_row = _step(step_index=1, label="Add favourites", view_id="None", bounds="10,10,30,30")
    dup1 = _step(step_index=2, label="Add favourites", view_id="", bounds="10,10,30,30")
    dup2 = _step(step_index=3, label="Add favourites", view_id="", bounds="10,10,30,30")
    dup3 = _step(step_index=4, label="Add favourites", view_id="", bounds="10,10,30,30")
    next_row = _step(step_index=5, label="Add device", view_id="id/title", bounds="20,20,40,40")
    client = _OverlayClient([post_click_row, dup1, dup2, dup3, next_row])
    tab_cfg = {"tab_name": "Home"}
    entry_step = _step(step_index=3, label="Add", view_id="id/add")
    rows: list[dict[str, str]] = []
    all_rows: list[dict[str, str]] = []
    ...
    assert [row["visible_label"] for row in overlay_rows] == ["Add favourites", "Add device"]
    assert overlay_rows[-1]["stop_reason"] == "overlay_max_steps"
