...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.11.1`)으로 관리하며, `moved/scrolled/looped` 포커스는 공통 가시성-포커스 루틴을 통해 하단 가림(`effectiveBottom-300`) 및 스크롤 직후 상단 정렬(`screenTop+300`) 보정을 수행합니다.
...
- `performSmartNext`는 새 타겟으로 이동하기 직전에 현재 `findFocus(FOCUS_ACCESSIBILITY)` 노드가 있으면 `ACTION_CLEAR_ACCESSIBILITY_FOCUS`를 먼저 보내 포커스 락을 끊습니다.
