...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.32.0`)으로 관리하며, `moved/scrolled/looped` 포커스는 공통 가시성-포커스 루틴에서 포커스 전 가시성 품질(fully visible, bottom clipping, trailing thin content)을 먼저 평가합니다. 하단 경계에 걸친 후보는 `ACTION_SHOW_ON_SCREEN` 및 최대 1회 pre-focus 정렬을 시도하고, 더 이상 스크롤이 어려운 마지막 컨텐츠는 fully-visible fallback으로 처리한 뒤 포커스를 진행합니다. 또한 스크롤 이후에는 pre-scroll anchor(뷰 ID/라벨/텍스트/좌표)를 이용해 연속성을 복원하고, anchor exact match 실패 시 continuation fallback 후보가 없으면 일반 post-scroll index=0 재진입을 하지 않고 즉시 스캔을 종료해 제어 흐름 꼬임을 방지합니다. 이때 히스토리 정책은 `visitedHistory`(실제 포커스 확인 이후에만 기록)와 `visibleHistory`(가시 이력)를 분리하고, continuation 후보는 단순 `visitedHistory=false`가 아니라 `pre-scroll anchor`의 logical successor, 스크롤 후 상단에 새로 등장한 미방문 content 후보, 하단 연속성(트레일링 카드), 하단 신규 노출 여부 순으로 평가합니다. 특히 `visibleHistory=true`라도 `visitedHistory=false`인 상단 신규 content 후보는 continuation으로 인정합니다. 추가로 설정 row 구간에서는 pre-scroll focused anchor의 logical successor를 최우선으로 해석해 상단 resurfaced 항목을 배제합니다. traversal 후보 구성 시 스크롤/루트 컨테이너(`ScrollView/NestedScrollView/RecyclerView`, `mainScrollView/content_container/root_container`)뿐 아니라 `feature_item_menu`/section/group/row/grid 래퍼와 다중 interactive child container를 일반 포커스 대상에서 제외해 설정 버튼 및 Supported devices 다음 후보가 실제 단일 카드로 이어지도록 보정합니다. 마지막 content 다음 순차 타겟이 bottom nav인 경우에도 스크롤 연속성 가능성(하단 경계 근접/클리핑/얇은 trailing content/연속 카드 패턴)을 먼저 확인하고, continuation 신호가 없을 때만 bottom nav로 즉시 전환합니다. snap-back 판정은 즉시 실패 대신 최대 250ms/50ms 간격 재확인 후, 추가로 350ms grace window를 적용해 지연 포커스를 재검증한 다음에만 최종 실패(`snap_back`)를 분류합니다.
...
- scroll 이후 후보 탐색은 pre-scroll anchor exact match를 먼저 시도하고, 실패하면 `findAnchorContinuationCandidateIndex(...)` 기반 continuation fallback을 사용합니다. 이때 우선순위는 `(1) pre-scroll focused anchor의 logical successor > (2) scroll 후 상단 신규(미방문) content continuation 후보 > (3) pre-scroll anchor 하단의 trailing continuation 후보 > (4) raw tree에는 있으나 traversal에서 누락됐던 설정 row(viewId 승격 대상) > (5) scroll 후 하단 신규 노출 본문 > (6) 기타 unvisited visible 후보` 순이며, 상단 resurfaced 브랜딩/anchor(`SmartThings`, `History` 등)는 제외(또는 최하위)합니다. 후보가 없을 때는 `[SMART_NEXT] Continuation fallback exhausted with no candidate` 및 `[SMART_NEXT] Skipping general post-scroll scan because continuation fallback failed` 로그를 남기고 일반 탐색 루프 재진입을 차단합니다. 또한 continuation context의 top `<no-label>` 후보는 즉시 노이즈 스킵하지 않고 descendant 라벨 복구/히스토리 재검사를 먼저 수행합니다.
...
- 리스트 끝에서 다음 후보가 하단 탭이면 `findAndFocusFirstContent(..., allowLooping = false)`로 새 콘텐츠 유무를 확인합니다. 단, continuation fallback 후보를 찾고도 확정하지 못한 케이스는 하단탭 fallback을 차단하고 `failed`로 종료해 오탐색 성공(`moved_to_bottom_bar`)을 방지합니다.
...
- `performFocusWithVisibilityCheck(...)`는 현재 포커스 바로 아래의 **첫 번째** partially visible 다음 후보를 intended candidate로 고정해(`[SMART_NEXT] Detected partially visible next candidate`) 후보 점프를 막고, pre-focus 보정은 `best effort minimal adjustment` 원칙으로 제한합니다. 즉 partially visible intended/후행 후보가 감지되면 컨테이너 강제 스크롤을 생략하고 `ACTION_SHOW_ON_SCREEN` 중심의 최소 보정만 수행하며, intended candidate가 fully visible이 되는 즉시 추가 보정을 중단합니다. 보정 결과 intended가 화면 상단 밖으로 밀리거나, 후행 카드의 가시 영역이 intended를 넘어 주 객체가 되면 overshoot로 기록(`[SMART_NEXT] Overshoot detected...`)하고 즉시 best-effort focus로 전환합니다.
