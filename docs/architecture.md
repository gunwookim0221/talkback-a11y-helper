...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.37.0`)으로 관리하며, `SMART_NEXT`는 단계 로그(`[COLLECT] → [NORMALIZE] → [SELECT_NEXT] → [PRE_SCROLL] → [POST_SCROLL_ANALYZE] → [CONTINUATION_SELECT] → [FOCUS_VERIFY] → [BOTTOM_BAR]`)를 남기는 파이프라인 구조로 정리되었습니다. 후보 선택 정책은 `selectSequentialNextCandidateIndex(...)`, `selectPostScrollContinuationCandidateIndex(...)`, `decideBottomBarEntry(...)`로 분리되어 회귀 시 원인 분리가 쉽습니다. 또한 post-scroll 포커스 루프는 `findAndFocusFirstContent(...)` 파일 스코프 helper와 `FindAndFocusPhaseContext`/`FindAndFocusRequest`로 분리되어, 오케스트레이션과 phase 실행 컨텍스트 전달이 명확해졌습니다. 기존과 동일하게 `visitedHistory`(실제 포커스 확인 이후만 기록)와 `visibleHistory`(가시 이력)를 분리하며, continuation 후보는 logical successor/신규 미방문 interactive(clickable·focusable, non-bottom-nav)/상단 신규 미방문 content/trailing continuation/하단 신규 노출 순으로 평가합니다.
...
- scroll 이후 후보 탐색은 pre-scroll anchor exact match를 먼저 시도하고, 실패하면 `findAnchorContinuationCandidateIndex(...)` 기반 continuation fallback을 사용합니다. 이때 우선순위는 `(1) pre-scroll focused anchor의 logical successor > (2) scroll 후 신규 미방문 interactive(clickable/focusable, non-bottom-nav) 후보 > (3) scroll 후 상단 신규(미방문) content continuation 후보 > (4) pre-scroll anchor 하단의 trailing continuation 후보 > (5) raw tree에는 있으나 traversal에서 누락됐던 설정 row(viewId 승격 대상) > (6) scroll 후 하단 신규 노출 본문 > (7) 기타 unvisited visible 후보` 순이며, 상단 resurfaced 브랜딩/anchor(`SmartThings`, `History` 등)는 제외(또는 최하위)합니다. `rewound before pre-scroll anchor` 신호는 hard reject가 아니라 저우선 힌트로 처리해, 신규 미방문 interactive 후보가 있으면 이를 우선 채택합니다. 후보가 없을 때는 `[SMART_NEXT] Continuation fallback exhausted with no candidate` 및 `[SMART_NEXT] Skipping general post-scroll scan because continuation fallback failed` 로그를 남기고 일반 탐색 루프 재진입을 차단합니다. 또한 continuation context의 top `<no-label>` 후보는 즉시 노이즈 스킵하지 않고 descendant 라벨 복구/히스토리 재검사를 먼저 수행합니다.
