...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.31.6`)으로 관리하며, `moved/scrolled/looped` 포커스는 공통 가시성-포커스 루틴에서 포커스 전 가시성 품질(fully visible, bottom clipping, trailing thin content)을 먼저 평가합니다. 하단 경계에 걸친 후보는 `ACTION_SHOW_ON_SCREEN` 및 최대 1회 pre-focus 정렬을 시도하고, 더 이상 스크롤이 어려운 마지막 컨텐츠는 fully-visible fallback으로 처리한 뒤 포커스를 진행합니다. 또한 스크롤 이후에는 pre-scroll anchor(뷰 ID/라벨/텍스트/좌표)를 이용해 연속성을 복원하고, anchor exact match 실패 시 continuation fallback 후보가 없으면 일반 post-scroll index=0 재진입을 하지 않고 즉시 스캔을 종료해 제어 흐름 꼬임을 방지합니다. 이때 히스토리 정책은 `visitedHistory`(실제 포커스 확인 이후에만 기록)와 `visibleHistory`(가시 이력)를 분리하고, continuation 후보는 단순 `visitedHistory=false`가 아니라 `pre-scroll anchor` 하단 연속성(트레일링 카드)과 하단 신규 노출 여부를 우선으로 평가합니다. 추가로 설정 row 구간에서는 pre-scroll focused anchor의 logical successor를 최우선으로 해석해 상단 resurfaced 항목을 배제합니다. traversal 후보 구성 시 스크롤/루트 컨테이너(`ScrollView/NestedScrollView/RecyclerView`, `mainScrollView/content_container/root_container`)와 화면 대부분을 덮는 대형 래퍼는 일반 포커스 대상에서 제외해 설정 버튼 다음 후보가 실제 콘텐츠 카드로 이어지도록 보정합니다. snap-back 판정은 즉시 실패 대신 최대 250ms/50ms 간격 재확인 후, 추가로 350ms grace window를 적용해 지연 포커스를 재검증한 다음에만 최종 실패(`snap_back`)를 분류합니다.
