...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.35.0`)으로 관리하며, `SMART_NEXT`는 단계 로그(`[COLLECT] → [NORMALIZE] → [SELECT_NEXT] → [PRE_SCROLL] → [POST_SCROLL_ANALYZE] → [CONTINUATION_SELECT] → [FOCUS_VERIFY] → [BOTTOM_BAR]`)를 남기는 파이프라인 구조로 정리되었습니다. 후보 선택 정책은 `selectSequentialNextCandidateIndex(...)`, `selectPostScrollContinuationCandidateIndex(...)`, `decideBottomBarEntry(...)`로 분리되어 회귀 시 원인 분리가 쉽습니다. 기존과 동일하게 `visitedHistory`(실제 포커스 확인 이후만 기록)와 `visibleHistory`(가시 이력)를 분리하며, continuation 후보는 logical successor/신규 미방문 interactive(clickable·focusable, non-bottom-nav)/상단 신규 미방문 content/trailing continuation/하단 신규 노출 순으로 평가합니다.
