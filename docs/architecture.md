...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.50.1`)으로 관리합니다. 또한 노드 판별/검색 유틸리티는 `A11yNodeUtils`로 분리되고, Smart Next 상태/결정/결과 모델은 `A11yModels`에서 통합 관리합니다. Smart Next `decideNextAction` 단계는 `NavigationDecision(REGULAR/PRE_SCROLL/BOTTOM_BAR/END)` 단일 결정으로 통합되며, 포커스 실행은 `requestFocusFlow` 단일 파이프라인(Pre-Focus → Action(최대 3회 재시도) → Verify(snap-back 체크))으로 유지됩니다.
