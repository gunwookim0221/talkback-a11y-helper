...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.8.5`)으로 관리하며, `moved/scrolled/looped` 포커스는 공통 가시성-포커스 루틴을 통해 하단 가림(`effectiveBottom-300`) 및 스크롤 직후 상단 정렬(`screenTop+300`) 보정을 수행합니다.
- `visibleHistory`는 일반 스크롤 콘텐츠만 저장하며, 상단바/하단바는 기록과 스크롤 후 히스토리 스킵 대상에서 모두 제외해 고정 UI를 항상 다시 탐색할 수 있게 유지합니다.
- 스크롤 대기 구간은 `Thread.sleep(700)` + `Thread.sleep(300)`만 유지하며, 더 이상 포커스 억제 루프나 새 포커스 직전 Double-Clear를 사용하지 않습니다.
