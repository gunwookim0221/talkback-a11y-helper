...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.30.6`)으로 관리하며, `moved/scrolled/looped` 포커스는 공통 가시성-포커스 루틴에서 포커스 전 가시성 품질(fully visible, bottom clipping, trailing thin content)을 먼저 평가합니다. 하단 경계에 걸친 후보는 `ACTION_SHOW_ON_SCREEN` 및 최대 1회 pre-focus 정렬을 시도하고, 더 이상 스크롤이 어려운 마지막 컨텐츠는 fully-visible fallback으로 처리한 뒤 포커스를 진행합니다. 또한 스크롤 이후에는 pre-scroll anchor(뷰 ID/라벨/텍스트/좌표)를 이용해 연속성을 복원하고, anchor exact match 실패 시 continuation fallback 후보가 없으면 일반 post-scroll index=0 재진입을 하지 않고 즉시 스캔을 종료해 제어 흐름 꼬임을 방지합니다. 이때 히스토리 정책은 `visitedHistory`(실제 포커스 성공)와 `visibleHistory`(가시 이력)를 분리해 continuation 후보가 단순 visible match만으로 제거되지 않게 합니다. snap-back 판정은 즉시 실패 대신 최대 250ms/50ms 간격 재확인 후, 추가로 350ms grace window를 적용해 지연 포커스를 재검증한 다음에만 최종 실패(`snap_back`)를 분류합니다.
...
- scroll 이후 후보 탐색은 pre-scroll anchor exact match를 먼저 시도하고, 실패하면 `findAnchorContinuationCandidateIndex(...)` 기반 continuation fallback으로 상단바/하단바 제외 + `visibleHistory(label)`/`visibleHistorySignature(label+viewId+bounds)` 미포함 본문 후보를 우선 선택합니다. 후보가 없을 때는 `[SMART_NEXT] Continuation fallback exhausted with no candidate` 및 `[SMART_NEXT] Skipping general post-scroll scan because continuation fallback failed` 로그를 남기고 일반 탐색 루프 재진입을 차단합니다. 또한 continuation context의 top `<no-label>` 후보는 즉시 노이즈 스킵하지 않고 descendant 라벨 복구/히스토리 재검사를 먼저 수행합니다.
