...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.11.4`)으로 관리하며, `moved/scrolled/looped` 포커스는 공통 가시성-포커스 루틴을 통해 하단 가림(`effectiveBottom-300`) 및 스크롤 직후 상단 정렬(`screenTop+300`) 보정을 수행합니다.
...
- `performSmartNext`는 현재 포커스 인덱스가 직전 성공 인덱스(`lastRequestedFocusIndex`)보다 작거나 같으면 stale 매칭으로 보고 `lastRequestedFocusIndex + 1`로 강제 전진해 동일 좌표 Grid 카드 루프를 끊습니다.
- `findAndFocusFirstContent(...)`는 현재 시스템 포커스와 좌표가 완전히 같은 후보를 라벨과 무관하게 skip하며, 특히 일반 이동(`isScrollAction=false`)에서는 동일 좌표 후보를 검증 없이 더 엄격하게 건너뜁니다.
- 같은 행(Y 좌표 동일)의 후보를 포커스할 때는 명령 직전 50ms 지연을 넣어 TalkBack의 물리적 이동 시간을 확보합니다.
- `performFocusWithVisibilityCheck(...)`는 포커스 요청 직전 `ACTION_CLEAR_ACCESSIBILITY_FOCUS`와 `TYPE_VIEW_ACCESSIBILITY_FOCUS_CLEARED`를 함께 보내 캐시를 비우고, 성공 판정의 모든 경로에서 `lastRequestedFocusIndex`를 빠짐없이 갱신해 다음 이동의 인덱스 기준점으로 사용합니다.
