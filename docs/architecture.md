...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.14.2`)으로 관리하며, `moved/scrolled/looped` 포커스는 공통 가시성-포커스 루틴을 통해 하단 가림(`effectiveBottom-300`) 및 스크롤 직후 상단 정렬(`screenTop+300`) 보정을 수행합니다. snap-back 강제 성공은 외부에 `moved_via_snap_back`으로 전달해 강제 성공 여부를 추적합니다.
...
- `performFocusWithVisibilityCheck(...)`에서 snap-back이 감지되면 내부 성공 처리는 유지하되 반환 status는 `moved_via_snap_back`으로 상세화해 강제 성공 경로를 추적합니다.
