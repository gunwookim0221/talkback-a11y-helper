...
또한 현재 Python runner(`script_test.py` + `tb_runner/*`)는 full-depth DFS 엔진이 아니라, 시나리오 기반 linear collector를 안정화하는 단계입니다.
- scenario 진입 안정화(tab/anchor/context)
- step 수집(`collect_focus_step`) + stop 정책
- Excel(`raw/filtered/summary`) 중심 데이터 정제
...
- `WebView`/비-leaf 컨테이너가 현재 포커스로 반복 보고되고 `currentIndex=-1`이 지속될 때는, 직전과 동일 컨테이너 시그니처 반복 + 후보 2개 이상 조건에서만 `[DECIDE][WEBVIEW_FIX]` 보정을 적용합니다. 이때 current는 `lastRequestedFocusIndex`(유효 시) 또는 `findClosestNodeBelowCenter(...)`로 좁게 복원하며, 매 호출마다 `[WEBVIEW_FIX][CHECK]`, `[WEBVIEW_FIX][CURRENT]`, `[WEBVIEW_FIX][NEXT]`, `[WEBVIEW_FIX][SKIP]` 로그를 고정 출력해 분기 진입 여부/사유 및 보정 결과를 추적합니다.
- `performSmartNext` 시작 직후 현재 focus가 `android.webkit.WebView` top-level container(`isAccessibilityFocused=true`, `childCount>0`)로 감지되면, 기존 traversal 진입 전에 WebView subtree에서 `visible + (focusable/screenReaderFocusable) + labeled + valid bounds` 조건을 만족하는 후보 중 top/left 우선 1개를 선택해 즉시 `ACTION_ACCESSIBILITY_FOCUS`를 수행합니다. 성공 시 `moved/webview_descend`로 조기 종료하고, 실패/후보 부재 시에는 기존 SMART_NEXT 로직으로 fallback합니다. 디버그 추적은 `[WEBVIEW_DESCEND] detected/candidate_count/selected_bounds/selected_label` 포맷으로 고정합니다.
- `performSmartNext` 시작 시 `[VERSION] appVersionName=... appVersionCode=... navigatorAlgorithmVersion=...` 로그를 출력해 실제 설치 APK의 버전과 helper 알고리즘 버전 반영 상태를 런타임 로그에서 확인합니다.
