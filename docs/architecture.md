...
- scroll 이후 후보 탐색은 pre-scroll anchor exact match를 먼저 시도하고, 실패하면 `findAnchorContinuationCandidateIndex(...)` 기반 continuation fallback으로 상단바/하단바 제외 + `visibleHistory` 미포함 본문 후보를 우선 선택합니다. fallback 후보가 현재 시스템 포커스와 동일 bounds면 duplicate-bounds 스킵을 우회하고 즉시 `moved` 성공으로 처리합니다.
