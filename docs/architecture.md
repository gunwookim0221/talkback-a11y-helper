...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.49.0`)으로 관리합니다. 또한 노드 판별/검색 유틸리티는 `A11yNodeUtils`로 분리되고, Smart Next 상태/결정/결과 모델은 `A11yModels`에서 통합 관리합니다. Smart Next 포커스 실행 단계는 `requestFocusFlow` 단일 파이프라인(Pre-Focus → Action(최대 3회 재시도) → Verify(snap-back 체크))으로 통합되었습니다.
