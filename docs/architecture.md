...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.50.0`)으로 관리합니다. 또한 노드 판별/검색 유틸리티는 `A11yNodeUtils`로 분리되고, 히스토리/억제 윈도우 관리는 `A11yHistoryManager`, 포커스 재시도/검증은 `A11yFocusExecutor`, post-scroll 후보 선택 진입점은 `A11yTraversalAnalyzer`로 분리합니다.
