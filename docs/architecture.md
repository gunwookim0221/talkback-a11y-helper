...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.14.2`)으로 관리하며, `moved/scrolled/looped` 포커스는 공통 가시성-포커스 루틴을 통해 하단 가림(`effectiveBottom-300`) 및 스크롤 직후 상단 정렬(`screenTop+300`) 보정을 수행합니다. snap-back 강제 성공도 외부에는 `moved`로 통일해 전달합니다.
...
- `performFocusWithVisibilityCheck(...)`에서 snap-back이 감지되면 내부 성공 처리는 유지하되 반환 status는 기존과 같은 `moved`로 통일해 외부 상태 판정 오탐을 막습니다.
