...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.38.0`)으로 관리하며, `performSmartNext()`는 `collectSmartNextRuntimeState(...) → decideNextAction(...) → executeNextAction(...) → verifyAndFinalizeNextAction(...)`의 오케스트레이션 파이프라인으로 고정되었습니다. 후보 선택 정책은 `selectSequentialNextCandidateIndex(...)`, `selectPostScrollContinuationCandidateIndex(...)`, `decideBottomBarEntry(...)`로 분리되어 회귀 시 원인 분리가 쉽습니다. 또한 post-scroll 포커스 루프는 `findAndFocusFirstContent(...)` 파일 스코프 helper와 `FindAndFocusPhaseContext`/`FindAndFocusRequest`로 분리되어, phase 실행 컨텍스트 전달이 명확해졌습니다. 기존과 동일하게 `visitedHistory`(실제 포커스 확인 이후만 기록)와 `visibleHistory`(가시 이력)를 분리하며, continuation 후보는 logical successor/신규 미방문 interactive(clickable·focusable, non-bottom-nav)/상단 신규 미방문 content/trailing continuation/하단 신규 노출 순으로 평가합니다.
