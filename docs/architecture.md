...
- scroll 이후 후보 탐색은 pre-scroll anchor exact match를 먼저 시도하고, 실패하면 `findAnchorContinuationCandidateIndex(...)` 기반 continuation fallback으로 상단바/하단바 제외 + `visibleHistory` 미포함 본문 후보를 우선 선택합니다. 새 본문이 없을 때만 하단탭 fallback을 허용합니다.
