...
- `visibleHistory`는 일반 스크롤 콘텐츠만 저장하며, 상단바/하단바는 기록과 스크롤 후 히스토리 스킵 대상에서 모두 제외해 고정 UI를 항상 다시 탐색할 수 있게 유지합니다. 또한 스크롤 직후 화면 최상단 200px 이내 후보는 히스토리에 있어도 재포커스를 허용해 시각적 흐름과 TalkBack 포커스 위치를 맞춥니다. 이때 대상 노드가 이미 시스템 접근성 포커스를 보유하면 추가 `ACTION_ACCESSIBILITY_FOCUS`를 생략해 중복 공지를 줄입니다.
