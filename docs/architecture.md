...
- 내비게이터 알고리즘 버전은 `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION`(현재 `2.9.5`)으로 관리하며, `moved/scrolled/looped` 포커스는 공통 가시성-포커스 루틴을 통해 하단 가림(`effectiveBottom-300`) 및 스크롤 직후 상단 정렬(`screenTop+300`) 보정을 수행합니다.
...
- 스크롤 후 히스토리 필터는 화면 상단 25% 이내에 들어온 후보를 우선 신뢰해, `Fixed UI`로 의심되더라도 상단 컨텐츠라면 히스토리를 무시하고 재포커스를 시도합니다. 메인 스크롤 컨테이너 구분은 유지하되, 상단 물리 위치 증거가 있을 때는 물리 위치를 더 강하게 반영합니다. 이때 대상 노드가 이미 시스템 접근성 포커스를 보유하면 추가 `ACTION_ACCESSIBILITY_FOCUS`를 생략해 중복 공지를 줄입니다.
