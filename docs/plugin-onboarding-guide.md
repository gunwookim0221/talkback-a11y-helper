...
## 15. V10 Shadow와 Onboarding Discovery의 관계

Plugin Onboarding Wizard의 `Discover Plugins`와 V10 Runtime Inventory는 목적이
다르다.

- Onboarding Discovery: 현재 visible card에서 신규 scenario draft seed를 만드는
  authoring workflow
- V10 Runtime Inventory: Full Run 이후 Devices surface를 bounded scan해 Shadow
  Identify 입력을 만드는 runtime workflow
- Quick Plugin Identify: card 진입 후 capability/XML evidence로 family candidate를
  반환하는 진단 단계
- Policy Registry: candidate를 기존 scenario id와 비교 가능한 형태로 매핑

Onboarding draft에는 여전히 실제 계정에서 관찰한 `target_stable_labels`가 필요하다.
V10 evidence는 display name을 semantic identity로 사용하지 않지만 Controlled
Routing이 없으므로 기존 scenario의 production entry contract를 자동으로 교체하지
않는다.
