...
**Evidence Ledger / Identity Shadow V2**

- 두 옵션은 기본 OFF이며 server process의 global environment가 아니라 Run마다 전달된다.
- `Identity Shadow V2`를 켜면 `Evidence Ledger`가 자동으로 켜진다.
- `Evidence Ledger`를 끄면 `Identity Shadow V2`도 자동으로 꺼진다.
- Backend는 해당 subprocess에만 `TB_EVIDENCE_LEDGER_ENABLED=1`과
  `TB_EVIDENCE_IDENTITY_SHADOW_ENABLED=1`을 전달한다.
- Run metadata의 `feature_flags.evidence_ledger`와
  `feature_flags.identity_shadow_v2`가 이후 history 표시의 provenance가 된다.
- 이 옵션은 physical focus evidence와 shadow verdict를 추가할 뿐 traversal, coverage,
  audit, summary, XLSX 또는 production PASS/WARN/FAIL을 바꾸지 않는다.
