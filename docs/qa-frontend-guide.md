...
**Evidence Ledger / Identity Shadow V2 / Traversal Identity V2**

- Evidence Ledger와 Identity Shadow V2는 Production Default Traversal Identity V2의 evidence prerequisite로
  run마다 함께 활성화된다. server process의 global environment는 사용하지 않는다.
- `Identity Shadow V2`를 켜면 `Evidence Ledger`가 자동으로 켜진다.
- `Traversal Engine`은 기본 ON이며, V2를 사용한다. OFF하면 해당 run은 Legacy Compatibility traversal을 사용한다.
- `Evidence Ledger`를 끄면 Identity와 Traversal도 자동으로 꺼지고, Identity를 끄면
  Traversal도 자동으로 꺼진다.
- Backend는 해당 subprocess에만 `TB_EVIDENCE_LEDGER_ENABLED=1`과
  `TB_EVIDENCE_IDENTITY_SHADOW_ENABLED=1`, `TB_TRAVERSAL_IDENTITY_V2_ENABLED=1`을
  기본 전달한다. Legacy Compatibility 선택 시 마지막 값은 명시적으로 `0`이다.
- Run metadata의 `feature_flags.evidence_ledger`와
  `feature_flags.identity_shadow_v2`, `feature_flags.traversal_identity_v2`가 이후 history
  표시의 provenance가 된다.
- Evidence/Identity만 ON이면 physical focus evidence와 shadow verdict만 추가한다.
- Production Default Traversal Identity V2에서는 strong closed transaction에 한해 progress,
  visit/consumed, repeat stop, conservative recovery가 opt-in으로 달라질 수 있다.
  Coverage 계산식, audit 의미, summary/XLSX schema는 바꾸지 않는다.
- Full Acceptance 기준 Safe recovery 개선, Motion parity, cross-plugin recovery가 확인됐다.
  Legacy 비교가 필요한 경우에만 Traversal Engine을 OFF로 선택한다.
...
## Identity Shadow V2 Card

Evidence ledger에 `SHADOW_ACTION_REDUCED_V2`가 있는 Device Details에서는 read-only
Identity Shadow V2 card를 열 수 있다. 기존 run에 V2 event가 없으면 `LEGACY_ONLY` 또는
unavailable로 표시하고 historical artifact를 재작성하지 않는다.

표시 항목:

- Transactions / Legacy-V2 Changed / Incomplete
- Strong physical / Insufficient
- `MOVE_CONFIRMED`, `STATIC_FOCUS`, `MOVE_TO_OTHER_NODE`, `SNAP_BACK`,
  `INDETERMINATE` count and percentage
- confidence count and percentage
- aggregate target relation count and percentage
- transaction별 Legacy/V2 verdict, target relation, temporal relation, confidence

필터는 disagreement, static focus, incomplete evidence를 조회하기 위한 UI 전용 기능이다.
Card와 API는 ledger의 recorded shadow result만 읽으며 traversal이나 production artifact를
변경하지 않는다.

## Traversal Identity V2 Diagnostics Card

`feature_flags.traversal_identity_v2=true`인 run의 Device Details에는 Experimental
diagnostics card가 표시된다. 이 card는 production result card를 대체하지 않고 ledger의
scenario diagnostics를 run 단위로 합산해 다음 값을 read-only로 표시한다.

- False progress suppressed
- Representative-only progress ignored
- Recovered candidate attempts / recovered visits
- Premature stop prevented
- Fallback to legacy / indeterminate

과거 run에 feature metadata가 없으면 unavailable, flag가 false면 disabled로 표시한다.
상세 gate와 Safe/Motion ON/OFF 절차는
`design/talkback-production-traversal-migration.md`를 참조한다.
...
- **Identity container evidence 제한**: current Safe/Motion ledger는 hierarchy path와
  bounded ancestor chain이 없어 container relation을 확정하지 못합니다. Bounds-only
  inference는 사용하지 않습니다.
- **Identity replay portability**: Frozen Safe/Motion ledger는 local gitignored artifact여서
  해당 파일이 없는 CI에서는 replay test가 skip됩니다.
- **Home Monitor Shadow FAIL 1건**: Full Acceptance에서 scenario FAIL은 0이었지만
  shadow-side Home Monitor mismatch 1건은 known limitation으로 남아 있습니다.
- **Identity container evidence 제한**: current Safe/Motion ledger는 hierarchy path와
  children inventory가 충분하지 않아 container relation 일부가 `INSUFFICIENT_EVIDENCE`로
  남습니다. bounds-only hierarchy inference는 사용하지 않습니다.
- **Positive OTHER_NODE/SNAP_BACK corpus 부족**: positive production corpus가 부족해
  `MOVE_TO_OTHER_NODE`, `SNAP_BACK`은 계속 diagnostic + legacy fallback입니다.
- **Legacy Compatibility**: Full Acceptance를 거쳐 Traversal Identity V2가 기본 엔진이 됐습니다.
  Legacy 비교가 필요할 때만 Run 화면에서 Traversal Engine을 OFF로 선택합니다.
...
- [Canonical Identity Shadow Phase 8 Completion](design/talkback-identity-shadow-phase8-completion.md)
