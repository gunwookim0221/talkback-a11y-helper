...
- 두 옵션은 기본 OFF이며 server process의 global environment가 아니라 Run마다 전달된다.
- `Identity Shadow V2`를 켜면 `Evidence Ledger`가 자동으로 켜진다.
- `Traversal Identity V2`를 켜면 앞의 두 옵션이 자동으로 켜진다.
...
- Traversal Identity V2까지 ON이면 strong closed transaction에 한해 progress,
  visit/consumed, repeat stop, conservative recovery가 opt-in으로 달라질 수 있다.
  Coverage 계산식, audit 의미, summary/XLSX schema는 바꾸지 않는다.
- Full Acceptance 기준 Safe recovery 개선, Motion parity, cross-plugin recovery는 확인됐지만
  이 옵션은 아직 default OFF experimental이다.
...
- **Traversal Identity V2 default OFF**: Full Acceptance는 통과했지만 run-scoped experimental
  feature flag이며 global default는 아직 OFF입니다.
