...
- `pre_navigation`: anchor 전에 수행할 bounded 이동(select/touch/scrollTouch). `scrollTouch`는 기본적으로 실행 직전에 `scroll_to_top`으로 best-effort 초기화한 뒤 검색을 시작하며, `new_screen` plugin 진입 시나리오에서는 한 step 내부에서 누적 downward 탐색을 수행합니다(초기 1회만 top reset).
  - 디버그 관측성: `scrollTouch` local search 단계에서 `[SCENARIO][pre_nav][scrolltouch][debug]`에 `rejections`, `pre_candidate_top`, `xml_fallback_attempted`, `xml_fallback_reason`이 포함되고, `[SCENARIO][pre_nav][scrolltouch][inspect]`에 상위 inspect sample(visible/clickable/promotion/reject reason)이 출력됩니다. 후보 선택 로그에는 `promoted_container`, `promotion_attempted`, `promotion_source`, `promotion_reason`, `promotion_candidate_count`, `promoted_from`, `promoted_to`가 포함되며, 스크롤 후 재탐색 로그에는 `settle_wait_ms`가 포함됩니다.
  - `life_air_care_plugin`의 실패 분석을 위해 `scrollTouch` 각 탐색 step마다 경량 캡처 번들(`screenshot.png`, `helper_dump.json`, `meta.json`)을 누적 저장하고, helper 승격 실패 직후 live XML fallback을 시도한 step에서는 같은 step 폴더에 `window_dump.xml`도 함께 저장합니다. 최종 실패 시에는 같은 run 폴더 아래 `final_failure`에 풀 번들(`window_dump.xml`, `focus_payload.json` 포함)을 추가 저장합니다.
  ...
  - 추가로 `before_pre_navigation` 루트 안정화에서 `life_root_not_stable`이 발생하더라도, `life_energy_plugin` 전용으로 **scrollTouch 진입 허용 gate**를 별도로 둡니다. Life 탭 선택(또는 하단 Life 노출) + 앱바 신호가 충분하고(`add/more options/location/qr code`) + `Navigate up`이 없으면, Energy 카드가 아직 안 보여도 pre-navigation(scrollTouch) 진입을 허용합니다. Family Care 시그니처는 리스트 카드 상태에서도 나타날 수 있으므로 단독 실패 근거로 사용하지 않습니다. 그 외에는 기존처럼 짧은 재관찰(최대 2회) 후 실패 처리합니다.
