...
- 디버그 관측성(기본 비활성화): 아래 항목은 **`LOG_LEVEL=DEBUG` + 명시적 플래그 true**를 동시에 만족할 때만 활성화됩니다(기본 `false`, 하위 호환용 `TB_LOG_LEVEL`도 지원).
  - `scrolltouch_debug_verbose_log_enabled=true` + `LOG_LEVEL=DEBUG`면 `scrollTouch` local search 단계에서 `[SCENARIO][pre_nav][scrolltouch][debug]`/`[inspect]` 로그(`rejections`, `pre_candidate_top`, `xml_fallback_attempted`, `xml_fallback_reason`, inspect sample 등)를 출력합니다.
  - `scrolltouch_debug_capture_enabled=true` + `LOG_LEVEL=DEBUG`면 scrollTouch 디버깅용 screenshot/helper dump/uiautomator XML/focus payload 캡처가 활성화되고, 이때에만 xml live fallback 보강 경로와 `output/capture_bundles/...` 저장이 동작합니다.
