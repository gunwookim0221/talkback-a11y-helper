...
- `pre_navigation`: anchor 전에 수행할 bounded 이동(select/touch/scrollTouch). `scrollTouch`는 기본적으로 실행 직전에 `scroll_to_top`으로 best-effort 초기화한 뒤 검색을 시작하며, `new_screen` plugin 진입 시나리오에서는 한 step 내부에서 누적 downward 탐색을 수행합니다(초기 1회만 top reset).
  - 디버그 관측성: `scrollTouch` local search 단계에서 `[SCENARIO][pre_nav][scrolltouch][debug]`에 `rejections`, `pre_candidate_top`이 포함되고, `[SCENARIO][pre_nav][scrolltouch][inspect]`에 상위 inspect sample(visible/clickable/promotion/reject reason)이 출력됩니다.
