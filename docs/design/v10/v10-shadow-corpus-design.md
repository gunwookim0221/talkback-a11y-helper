...
| Metadata | Value |
| --- | --- |
| Status | Implemented (Storage + Analytics Dashboard) |
| Phase | V10 Follow-up |
...
QA Frontend의 `V10 Corpus Readiness`는 위 세 summary를 읽기 전용으로 결합한다.
`GET /api/v10/corpus/summary` 응답은 entry 수, 마지막 갱신 시각, 전체 readiness,
comparison 누계, family table, 다양성 지표, blocking/unknown-only family와 기존
`candidate_for_v11_pilot` 목록을 제공한다. 저장 schema와 candidate 판정은 변경하지
않는다.

다음 endpoint는 고정된 corpus 위치만 로컬 기본 프로그램으로 연다.

```text
POST /api/v10/corpus/open/folder
POST /api/v10/corpus/open/index
POST /api/v10/corpus/open/family-summary
POST /api/v10/corpus/open/readiness-summary
```
...
- Analytics API/UI는 summary JSON이 모두 있어야 data available로 표시한다. 파일이
  없거나 유효한 JSON이 아니면 `No corpus data`로 표시하며 자동 rebuild하지 않는다.
- Dashboard는 읽기 전용이며 Controlled Routing action을 제공하지 않는다.
