...
- `script_test.py` 러너(`SCRIPT_VERSION=1.6.1`)는 탭 진입 직후 anchor를 바로 신뢰하지 않고 안정화 단계를 수행합니다.
...
- `context_verify`는 시나리오별 optional 설정이며 미설정(또는 `type: none`) 시 기존과 동일하게 동작합니다.
  - `selected_bottom_tab`: **현재 focus payload가 아닌 dump_tree_nodes 기반** 하단 탭 선택 문맥 검증 (`selected=true` 또는 `Selected|선택됨` + Home/Devices/...)
