...
object A11yNavigator {
    ...
    private fun <T> selectPostScrollContinuationCandidateResult(
        ...
        val expectedSuccessorViewId = normalizedAnchorViewId
            ?.let { A11yTraversalAnalyzer.SETTINGS_ROW_VIEW_ID_ORDERED.indexOf(it) }
            ?.takeIf { it >= 0 && it < A11yTraversalAnalyzer.SETTINGS_ROW_VIEW_ID_ORDERED.lastIndex }
            ?.let { A11yTraversalAnalyzer.SETTINGS_ROW_VIEW_ID_ORDERED[it + 1] }
    ...
    internal fun requestFocusFlow(
        ...
        if (!focusExecution.success) {
            syncLastRequestedFocusIndexToCurrentFocus(root, A11yTraversalAnalyzer.buildTalkBackLikeFocusNodes(root).map { it.node })
    ...
    internal fun recordRequestedFocusAttempt(index: Int, root: AccessibilityNodeInfo? = null) {
        ...
        if (root != null && focusedNode != null) {
            val traversalNodes = A11yTraversalAnalyzer.buildTalkBackLikeFocusNodes(root).map { it.node }
