...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.9.4`
  ...
  - `isFixedSystemUI`는 노드/조상에 `Toolbar`·`ActionBar`·`BottomNavigationView` 계열 식별자가 있거나, 메인 스크롤 컨테이너 바깥에 존재하거나, `Add`/`More options` 계열 시스템 버튼이 스크롤 영역 밖에 있을 때 고정 UI로 분류
  - 스크롤 직전 화면에 보인 라벨을 `visibleHistory`로 저장하되, 스크롤 후 히스토리 필터에서는 `Fixed UI`를 항상 스킵하고 메인 스크롤 컨테이너 내부 일반 콘텐츠만 검사
  - 스크롤 후 동일 라벨이라도 메인 스크롤 컨테이너 내부 컨텐츠가 화면 상단 25% 이내(`bounds.top <= screenTop + screenHeight * 0.25`)에 있으면 히스토리를 무시하고 재포커스를 허용
