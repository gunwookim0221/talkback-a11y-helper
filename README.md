...
- `A11yNavigator`
  ...
  - `A11yFocusExecutor.VERSION = 1.3.9`: `requestAccessibilityFocusWithRetry(...)`에 Sniper 로직을 적용해 텍스트 병합 부모(껍데기) 대신 내부의 실제 상호작용 가능한 자식(`isClickable || isFocusable`)을 재귀 탐색해 `ACTION_ACCESSIBILITY_FOCUS`를 직접 요청하도록 수정했습니다. 또한 검증 단계에서 실제 포커스 bounds가 타겟 bounds 내부에 포함되는 경우도 성공으로 인정해 Silent Reject를 완화했습니다.
