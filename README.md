...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.5.1`: `executeClickFromFocusedNode<T>`의 wrapper recovery queue 초기화에서 nullable `rootNode: T?`가 non-null queue 원소(`T`)로 직접 전달되던 컴파일 오류를 null-check(`rootNode?.let(queue::add)`)로 보정했습니다. 기존 클릭 경로/우선순위 로직은 변경하지 않았습니다.
