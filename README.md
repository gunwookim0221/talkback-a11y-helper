...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.8.7`
  ...
  - 스크롤 대기 구간은 최대 1.5초 동안 100ms 간격으로 `collectVisibleHistory(rootInActiveWindow)` 변화를 폴링해 트리 갱신을 감지하고, 갱신 즉시 최신 `rootInActiveWindow`로 재탐색해 TalkBack보다 먼저 새 객체를 선점
