...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.14.2`
  ...
  - `ACTION_ACCESSIBILITY_FOCUS` 직전 `target.refresh()`로 최신 상태를 다시 읽고, 성공 직후 추가 50ms를 대기해 파란 포커스 테두리와 시스템 포커스 좌표의 안착을 유도
  - 포커스 전 `ACTION_SHOW_ON_SCREEN`이 실행되면 100ms를 추가 대기해 큰 카드가 완전히 안착한 뒤 포커스를 시도
  - `ACTION_ACCESSIBILITY_FOCUS`는 동일 `performSmartNext` 호출 안에서 타겟당 1회만 실행하며, 반환값이 `false`여도 직후 `isAccessibilityFocused=true`이면 TalkBack이 선점한 것으로 간주해 성공 처리
  ...
  - `performFocusWithVisibilityCheck(...)`는 `ACTION_ACCESSIBILITY_FOCUS` 직전/직후에 시스템 `findFocus`와 타겟의 전체 좌표를 `Before Focus`/`After Focus` 로그로 남겨 실제 시스템 포커스 이동 여부를 비교 진단하고, 포커스 직후에는 50ms를 추가 대기해 좌표 갱신 안착 시간을 확보
  ...
  - snap-back이 발생하면 `[SMART_NEXT] Snap-back handled: Forcing success to maintain sequence at index X` 로그를 남기고, 반환 status는 `moved`로 통일합니다. 또한 `lastRequestedFocusIndex`/`A11yStateStore.lastRequestedFocusIndex`에는 이미 시도한 인덱스와 실제 시스템 포커스 좌표 기준 인덱스를 함께 반영한 뒤 반환 직전 100ms 대기해 TalkBack 발화 시간을 확보
