...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.9.1`
  ...
  - 스크롤 후 동일 라벨은 일반 스크롤 콘텐츠에 대해서만 스킵하되, 스크롤 직후 화면 최상단 200px 이내(`bounds.top < screenTop + 200`)에 있는 후보는 히스토리에 있어도 재포커스를 허용하고 상단바/하단바도 계속 탐색 가능하도록 유지
