...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.68.5`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.10.5`: OneConnect Settings의 `update_app_card` + `update_app_title` alias 판정을 더 엄격한 exact viewId + bounds/ancestor 조건으로 고정하고, normalize 대표를 `update_app_card`로 일관화해 card/title 교대 관측 시에도 1회 logical object로만 소비되도록 보강했습니다.
