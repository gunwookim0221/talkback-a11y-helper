...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.4.6`: `CLICK_FOCUSED`에서 raw focused node 재해석 성공 후 subtree child 접근이 비어도(`childCount=0`) `focused bounds` 주변 local raw search를 추가로 수행해 top-right small actionable(예: `settings_image`)를 mirror candidate source에 주입합니다. `click_focused_descendant_scan_start`, `click_focused_local_raw_search_start`, `source='local_raw_search'`, stage=`local_raw_collected` 로그를 추가했고 giant/top/locality/global fallback 정책은 유지합니다.
