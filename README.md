...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.65.0`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.7.0`: wrapper duplicate 판별을 위해 `label 유사도 + bounds 강한 중첩/포함 + 동일 package + 계층(ancestor)` 기반 alias 판정 로직을 추가하고, 독립 action control(button/switch/toggle 등) 및 독립 interactive descendant가 있는 케이스는 병합에서 제외하도록 보강했습니다.
