...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.4.4`: `CLICK_FOCUSED` mirror candidate discovery 단계에 `focused vicinity candidate set`을 추가했습니다. focused node가 non-clickable일 때 raw descendant를 강제 BFS 탐색해 clickable + class(ImageButton/Button/ViewGroup) + bounds(포커스 내부/근접) 조건을 만족하는 후보를 별도로 수집하고, 해당 수집 로그(`[click_focused_descendant_candidate_seen]`)를 남깁니다. 기존 giant container reject, top-region/dx/dy 필터, scoring 로직은 유지합니다.
