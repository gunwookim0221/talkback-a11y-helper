...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.4.1`: `CLICK_FOCUSED` mirror resolve 스코어링을 로컬 밴드 우선 + giant container(예: ScrollView/RecyclerView/대면적 FrameLayout) 강한 배제 정책으로 조정해, 상단의 작은 focused target이 화면 대부분을 덮는 컨테이너로 잘못 매핑되는 문제를 줄였습니다. `click_focused_mirror_candidates`/`click_focused_mirror_reject` 로그로 giant 필터링 관측치도 추가했습니다.
