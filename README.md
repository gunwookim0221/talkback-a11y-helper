...
- `A11yHelperService`
  ...
  - GET_FOCUS 스냅샷 스키마 버전: `schemaVersion = 1.1.0` (신규 `mergedLabel`, `talkbackLabel`, `visibleToUser`, `children` 포함)
...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.14.5`
  ...
  - `SMART_NEXT`가 `ACTION_SCROLL_FORWARD` 후 새 traversal 스냅샷이 이전 화면과 100% 동일하면 `[SMART_NEXT] No scroll progress detected, terminating to prevent loop` 로그를 남기고 즉시 `reached_end_no_scroll_progress`로 종료해 무한 스크롤 루프를 차단
  ...
  - 리스트 마지막/다음 인덱스 미존재 상황에서는 스크롤 가능한 컨테이너가 있으면 항상 `ACTION_SCROLL_FORWARD`를 먼저 시도하고, 스크롤 대상이 없으면 `[SMART_NEXT] Reached end of list, stopping traversal` 로그와 함께 `reached_end`를 반환해 자동 루핑을 중단
  ...
  - `performFocusWithVisibilityCheck(...)`는 `ACTION_ACCESSIBILITY_FOCUS` 직전/직후에 시스템 `findFocus`와 타겟의 전체 좌표를 `Before Focus`/`After Focus` 로그로 남겨 실제 시스템 포커스 이동 여부를 비교 진단하고, 포커스 직후에는 150ms를 추가 대기해 좌표 갱신 안착 시간을 확보합니다. 이 함수에서 성공 시 반환하는 status는 경로와 무관하게 모두 `moved`로 정규화됩니다
  - `ACTION_ACCESSIBILITY_FOCUS` 직전에는 `ACTION_FOCUS`(입력 포커스)와 이중 `ACTION_CLEAR_ACCESSIBILITY_FOCUS`를 함께 시도해 시스템 거부 반응을 줄이고, 성공 반환 뒤 실제 시스템 포커스 중심점이 타겟 중심점에서 10px 이내면 성공으로 간주합니다. 10px를 넘겨 벗어날 때만 snap-back으로 판정하되 내부적으로는 해당 이동을 성공 처리해 한 칸씩만 전진
  - snap-back이 발생하면 `[SMART_NEXT] Snap-back handled: Forcing 'moved' status to maintain client compatibility` 경고 로그를 남기되, 외부 반환 status는 일반 성공과 동일한 `moved`로 통일합니다. 또한 `lastRequestedFocusIndex`/`A11yStateStore.lastRequestedFocusIndex`에는 이미 시도한 인덱스와 실제 시스템 포커스 좌표 기준 인덱스를 함께 반영한 뒤 반환 직전 100ms 대기해 TalkBack 발화 시간을 확보
