...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.11.6`
  ...
  - `performSmartNext`는 현재 노드와 `Rect.equals`로 완전히 같은 좌표를 가진 후속 후보를 `while` 루프로 연속 스킵하고, `[SMART_NEXT] Skipping coordinate duplicate: jumping from X to Y` 로그를 남기며 실제 물리 위치가 바뀌는 다음 타겟으로 즉시 점프
