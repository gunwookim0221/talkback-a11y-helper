...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.68.9`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.10.6`: OneConnect Settings의 `update_app_card` 특수 케이스를 `update_app_title/update_app_text/update_app_card_close_btn/update_button`까지 exact viewId + bounds 포함 + card-ancestor 조건으로 확장해 카드 내부 멤버를 모두 동일 logical object로 묶고 대표를 항상 `update_app_card`로 고정했습니다.
  - `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION = 2.68.9`: `decideInitialNextTarget(...)`가 normalized traversal 기준으로만 인덱스를 계산/보정하도록 통일해 OneConnect Settings에서 `Update app` 다음 `Samsung account`가 스킵되며 `Notifications`로 점프하는 문제를 줄였습니다. 또한 `Notifications` same-row reselection 차단에서 strict representative visited 여부와 무관하게 동일 row 재선택을 막고, regular single-target 실패 시 동일 Notifications row 재실패 상황에서만 다음 external candidate 1회 재시도를 허용해 `repeat_no_progress` 루프를 완화했습니다.
