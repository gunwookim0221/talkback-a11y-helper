...
- `A11yNavigator`
  ...
  - `A11yFocusExecutor.VERSION = 1.3.4`: `alignCandidateForReadableFocus(...)`의 pre-focus 보정에서 `ACTION_SHOW_ON_SCREEN` 실제 성공 여부로만 조정 상태를 갱신하고, 성공 시 컨테이너 `ACTION_SCROLL_FORWARD`를 생략해 이중 스크롤 충돌을 방지합니다. 또한 보정 후 대기를 400ms로 늘리고 `target.refresh()` 뒤 bounds를 읽어 스크롤 애니메이션 중 snap-back 오탐을 줄였습니다.
