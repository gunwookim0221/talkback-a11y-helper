...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.74.2`
  ...
  - regular single-target 경로에서 `next candidate`가 소진된 상태(동일/역행 인덱스)로 실패하고, 실패 직후 시스템 포커스가 current와 동일하게 반복되면 `[END_CHECK] candidate_count/next_candidate/scroll_attempted/scroll_changed` 로그를 남깁니다. 이때 스크롤이 불가능하거나 스크롤 후 후보 변화가 없으면 `[END_CHECK] decision=looped`로 종료해 `failed` 반복을 방지합니다.
