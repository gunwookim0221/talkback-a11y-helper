...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.63.0`
  ...
  - `A11yFocusExecutor.VERSION = 1.4.3`: single-target 모드는 유지하면서 attempt/post_action_settle의 match 판정을 동일 기준(실제 focused node + bounds + target bounds + target package)으로 통일했습니다. polling 중 `com.android.systemui`/외부 package 포커스는 즉시 실패 확정 대신 skip 처리하고, attempt window에서 한 번이라도 match가 성립하면 settle mismatch로 success를 덮어쓰지 않도록 고정했습니다(명백한 외부 package 이탈 신호는 별도 실패 처리).
