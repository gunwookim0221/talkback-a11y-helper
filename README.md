...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.9.5`
  ...
  - 스크롤 직전 화면에 보인 라벨을 `visibleHistory`로 저장하되, 스크롤 후 히스토리 필터는 화면 상단 25% 이내에 도달한 후보를 먼저 신뢰합니다
  - 스크롤 후 동일 라벨이라도 후보가 화면 상단 25% 이내(`bounds.top < screenTop + screenBottom / 4`)라면 `Fixed UI` 의심 여부보다 먼저 히스토리를 무시하고 재포커스를 허용
