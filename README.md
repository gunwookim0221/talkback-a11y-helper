...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.7.5`
  ...
  - 스크롤 후(`isScrollAction=true`) 직전 항목 제외 조건(`excludeDesc`)으로 탐색했는데 유효 포커스를 찾지 못하면, 리스트 종단으로 간주해 `excludeDesc`를 유지한 채 `startIndex=0`, `status=looped`로 즉시 1회 루핑 재탐색
  - `SMART_NEXT` 스크롤 후 대기 시간을 1500ms + 추가 200ms로 확보하고, 남아 있는 이전 접근성 포커스(Focus Lock)를 `ACTION_CLEAR_ACCESSIBILITY_FOCUS`로 강제 해제한 뒤 새 콘텐츠 포커스를 재배정
  ...
  - 스크롤 후 루프 폴백(`status=looped`) 재탐색에서도 직전 pass의 `excludeDesc`를 유지해 동일 항목 재포커스를 방지(단, 상단 30% 1회 스킵 가드는 기존과 동일 유지)
