...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.28.0`
  ...
  - scroll 이후 anchor exact match가 실패하면 `[SMART_NEXT] Anchor exact match failed; using continuation fallback` 로그 후 `findAnchorContinuationCandidateIndex(...)`로 `startIndex=0` 전체를 재탐색하며, 상단바/하단바를 제외하고 `visibleHistory`에 없는 첫 본문 후보를 우선 선택
  - 위 continuation fallback에서 새 본문 후보를 찾으면 `[SMART_NEXT] Selected new post-scroll content candidate ...`를 기록하고, 끝까지 없을 때만 `[SMART_NEXT] No new continuation content found; allowing bottom bar` 로그와 함께 하단탭(Home 등) 진입을 허용
  ...
  - 위 `<no-label>` continuation 후보는 descendant readable text를 재수집해 라벨 복구를 시도하며, 복구 시 `[SMART_NEXT] Recovered descendant label for continuation candidate: ...` 로그를 남김 (예: `Labs`)
  - 기존 `Skipping top <no-label> ...` 규칙은 일반 상단 노이즈 필터에만 적용되고, post-scroll continuation fallback이 고른 후보에는 적용하지 않음. 일반 상단 노이즈는 `[SMART_NEXT] Skipping top <no-label> noise node` 로그로 분리
