...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.70.0`
  ...
  - `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION = 2.70.0`: OneConnect Settings 한정 generic row grouping(`row container/title/summary/switch`)을 추가해 Notifications뿐 아니라 Sync favourites 같은 동일 logical row의 반복 소비를 통합 차단했습니다. representative는 가까운 row container로만 고정하고, wrapperLayout/ScrollView/상위 컨테이너는 depth+bounds 조건으로 제외하며 `[DEBUG][ROW_GROUP] ...` 로그를 남깁니다.
