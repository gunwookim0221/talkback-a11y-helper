...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.14.0`
  ...
  - `performFocusWithVisibilityCheck(...)`는 `ACTION_ACCESSIBILITY_FOCUS` 직전/직후에 시스템 `findFocus`와 타겟의 전체 좌표를 `Before Focus`/`After Focus` 로그로 남겨 실제 시스템 포커스 이동 여부를 비교 진단
  ...
  - snap-back이 발생하면 `[SMART_NEXT] Snap-back handled: Forcing success to maintain sequence at index X` 로그를 남기고, `lastRequestedFocusIndex`/`A11yStateStore.lastRequestedFocusIndex`에는 이미 시도한 인덱스를 유지한 채 반환 직전 100ms 대기해 TalkBack 발화 시간을 확보
