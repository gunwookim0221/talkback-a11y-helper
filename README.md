...
- `A11yNavigator`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.8.4`: collect 단계에서 broad하게 trailing action control 후보를 보강하던 방식을 제거하고, normalize 이후 `multiline composite row` 대표 노드 바로 뒤에만 trailing switch 계열을 좁게 삽입하는 logical stop 후처리(`TRACE_TRAILING_STOP_INSERT`)로 전환했습니다. row-first 순서를 유지하면서 alias/visited 충돌을 줄이고 single-target 이동 제약을 유지합니다.
