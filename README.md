...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.68.2`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.10.3`: OneConnect package 한정으로 `focusable/clickable`이 아닌 `TextView` 정적 본문을 구조 조건(ancestor section 내부, interactive 아님, top/bottom chrome 제외, 유효 bounds)으로만 제한 승격해 Guided cooking 이후 step 텍스트가 traversal 후보에 이어지도록 보정했습니다. 동시에 `static_text_included/static_text_rejected` 로그로 승격/제외 사유를 기록합니다.
...
- Python 클라이언트 알고리즘 버전: `CLIENT_ALGORITHM_VERSION = 1.7.41`
- 발화 조회 API
