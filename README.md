...
- `A11yNavigator`
  ...
  - `performFocusWithVisibilityCheck(...)`는 `ACTION_ACCESSIBILITY_FOCUS` 직전/직후에 시스템 `findFocus`와 타겟의 전체 좌표를 `Before Focus`/`After Focus` 로그로 남겨 실제 시스템 포커스 이동 여부를 비교 진단하고, 포커스 직후에는 150ms를 추가 대기해 좌표 갱신 안착 시간을 확보
  ...
  - snap-back이 발생하면 `[SMART_NEXT] Snap-back handled: Forcing success to maintain sequence at index X` 로그를 남기고, 반환 status는 `moved_via_snap_back`으로 상세화합니다. 또한 `lastRequestedFocusIndex`/`A11yStateStore.lastRequestedFocusIndex`에는 이미 시도한 인덱스와 실제 시스템 포커스 좌표 기준 인덱스를 함께 반영한 뒤 반환 직전 100ms 대기해 TalkBack 발화 시간을 확보
