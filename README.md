...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.5.0`: `CLICK_FOCUSED`에서 accessibility-focused wrapper가 `clickable=false`, `childCount=0`으로 관측되는 런타임 케이스를 위해 bounds 기반 wrapper recovery fallback을 추가했습니다. direct/descendant/mirror resolve 기존 순서는 유지하고, 조건(접근성 포커스 + focusable + non-clickable + empty subtree + top small target) 충족 시에만 top band 내부 clickable 후보를 bounds 관계(내부 포함 > 강한 overlap > 중심 근접)로 선별합니다. `click_focused_wrapper_recovery_start/candidate/pick/reject` 로그를 추가했고 giant container 및 far/global 오탭 방지 정책은 유지합니다.
