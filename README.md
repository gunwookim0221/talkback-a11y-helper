...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.7.4`
  ...
  - 스크롤 후(`isScrollAction=true`) 직전 항목 제외 조건(`excludeDesc`)으로 탐색했는데 유효 포커스를 찾지 못하면, 리스트 종단으로 간주해 `excludeDesc=null`, `startIndex=0`, `status=looped`로 즉시 1회 루핑 재탐색
