...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.70.2`
  ...
  - `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION = 2.70.2`: OneConnect Settings generic row grouping이 Notifications row까지 담당하도록 normalize/대표선정/visited alias 처리를 정리하고, Notifications 특수 same-row 차단은 generic 미적용 시에만 동작하는 fallback으로 축소했습니다. `update_app_card` 특수 케이스 및 wrapperLayout/ScrollView/상위 컨테이너 제외 규칙은 유지됩니다.
