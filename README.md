...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.68.4`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.10.4`: OneConnect Settings의 `update_app_card` + `update_app_title` 조합을 alias duplicate로 묶도록 좁은 조건을 추가해 동일 logical object 반복 소비를 차단했습니다(패키지/뷰ID/ancestor-chain/overlap 동시 충족 시에만 적용).
