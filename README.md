...
- `A11yNavigator`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.8.3`: collect 단계에서 `multiline composite row` 내부의 trailing 독립 action control(예: checkable switch)이 direct clickable/focusable이 아니어도 구조 조건(독립 action class + checkable/interactive 속성 + parent의 다중 라벨 + trailing bounds)을 만족하면 별도 후보로 보강 수집하도록 수정했습니다. 이로써 parent row 다음에 trailing switch가 normalize 후보에 유지되며, dedupe/empty-shell/alias 규칙은 변경하지 않았습니다.
