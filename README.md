...
- `A11yCommandReceiver`
  - 브로드캐스트 액션 처리
    ...
    - `com.iotpart.sqe.talkbackhelper.SCROLL` (`forward` boolean)
...
- `A11yNavigator`
  ...
  - `targetType`: `t`(text contains/regex), `b`(contentDescription contains/regex), `r`(viewIdResourceName regex 매칭, 일반 문자열은 exact 매칭), `a`(앞 3개 OR)
...
```bash
adb shell am broadcast -a com.iotpart.sqe.talkbackhelper.GET_FOCUS -p com.iotpart.sqe.talkbackhelper --ez saveFile true
```
...
```bash
adb shell am broadcast -a com.iotpart.sqe.talkbackhelper.SCROLL -p com.iotpart.sqe.talkbackhelper --ez forward true
adb shell am broadcast -a com.iotpart.sqe.talkbackhelper.SCROLL -p com.iotpart.sqe.talkbackhelper --ez forward false
```
