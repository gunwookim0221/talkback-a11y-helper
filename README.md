...
- `A11yNavigator`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.8.2`: Settings 계열의 설명 텍스트가 포함된 composite row에서 trailing `Switch/Button`이 무라벨이라는 이유로 empty-shell 단계에서 제거되던 케이스를 보정했습니다. 부모 composite row 후보는 유지하면서도, 우측 trailing action control을 별도 candidate로 함께 보존해 `parent → trailing control` 순서를 유지합니다(단일 호출 단일 타깃/일반 경로 later sweep 금지 정책 유지).
