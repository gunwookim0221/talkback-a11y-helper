...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.8.8`
  ...
  - 스크롤 대기 구간은 최대 1.5초 동안 100ms 간격으로 새 `rootInActiveWindow`에서 `buildFocusableTraversalList`와 `collectVisibleHistory(...)`를 다시 계산해 트리 갱신을 감지하고, 갱신 즉시 최신 루트로 재탐색해 TalkBack보다 먼저 새 객체를 선점
