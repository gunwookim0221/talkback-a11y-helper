...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.4.3`: `CLICK_FOCUSED` mirror resolve 파이프라인을 `candidate 수집 → giant filter → top-region 판정 → locality 판정` 순서로 재정렬하고, 상단 소형 타겟에서 top-band 내부 clickable 후보를 locality보다 우선 통과시키도록 조정했습니다. giant container 배제/글로벌 fallback 금지는 유지하면서, far body leaf는 계속 탈락시키고 toolbar clickable 후보 복구를 위해 stage/reject 진단 로그(`[click_focused_candidate_seen]`, `[click_focused_candidate_pass_stage]`, `[click_focused_candidate_reject]`)를 추가했습니다.
