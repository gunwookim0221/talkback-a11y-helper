...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.8.4`
  ...
  - 노드 포커스 직전 `bounds.bottom > (effectiveBottom - 300)`이면 하단 탭에 가려진(clipping) 상태로 판단하고 API 26+에서는 `AccessibilityNodeInfo.AccessibilityAction.ACTION_SHOW_ON_SCREEN.id`를 호출해 하단 카드(예: `Pet Care`)를 더 위로 끌어올림
  - 스크롤 직전 화면에 보인 라벨을 `visibleHistory`로 저장하되, 상단바/하단바(`isTopAppBarNode`/`isBottomNavigationBarNode`)는 기록에서 제외해 고정 UI가 방문 이력 때문에 사라지지 않도록 함
  - 스크롤 후 동일 라벨은 일반 스크롤 콘텐츠에 대해서만 스킵하고, 상단바/하단바는 히스토리에 있더라도 계속 탐색 가능하도록 유지
  - 스크롤 직후 1초 대기 대신 100ms 간격 × 10회 포커스 가디언 루프를 실행해 자동 선점 포커스(예: `Clothing Care`)를 지속 억제
  ...
  - 스크롤 후 제외 대상(`excludeDesc`) 노드가 여전히 접근성 포커스를 유지 중이면, 탐색 루프 진입 전에 `ACTION_CLEAR_ACCESSIBILITY_FOCUS`로 강제 해제하여 중복 공지를 차단
  - 새 후보 노드 포커스 직전에 현재 접근성 포커스를 한 번 더 즉시 해제한 뒤, 50ms 지연 없이 즉시 `ACTION_ACCESSIBILITY_FOCUS`를 수행
  - v2.8.2에서는 스크롤 대기구간에 포커스 가디언을 적용하고, 포커스 시도 속도를 높여 루프 폴백 시에도 실제 첫 번째 콘텐츠 탐색을 우선
  - `ACTION_ACCESSIBILITY_FOCUS`가 거부되면 100ms 대기 후 1회 재시도하여 스크롤 애니메이션 잔여 구간의 포커스 실패를 완화
  ...
  - 상단 고정 영역 판별: 클래스(`toolbar/actionbar/appbarlayout`) + 리소스 ID 키워드(`title_bar/header/toolbar/more_menu/action_bar/home_button/tab_title/header_bar`)만 사용(좌표 기반 15% 임계값 제거)
  - 하단 고정 영역 판별: 클래스(`bottomnavigation/tablayout/navigationbar`) + 리소스 ID 키워드(`bottom/footer/tab_bar/navigation/menu_bar/menu_favorites/menu_devices/bottom_menu/bottom_tab/bottom_nav`)만 사용(좌표 기반 15% 임계값 제거)
  - 하단 경계의 리스트 카드(예: `Pet Care`)가 잘못 스킵되지 않도록 하단바 판별은 클래스/ID 키워드만 사용하고 좌표 기반(bottom 20%) 오인식 로직을 배제
  - `findAndFocusFirstContent` 시작 시 현재 윈도우의 접근성 포커스를 즉시 `ACTION_CLEAR_ACCESSIBILITY_FOCUS`로 해제하고 `[SMART_NEXT] Interrupting TalkBack auto-focus on: ...` 로그로 자동 포커스 개입을 차단
  - 포커스 시도 직전에 `[SMART_DEBUG] Attempting focus on Index:$index, AlreadyFocused:${node.isAccessibilityFocused}` 로그를 남겨 시스템 선점 여부를 추적
  - 포커스 성공 직후 노드 Top이 `screenTop + 300px`보다 아래면 `ACTION_SHOW_ON_SCREEN`을 추가 호출해 신규 항목(예: `Home Care`)을 화면의 진짜 최상단에 가깝게 정렬
