...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.30.7`
  ...
  - scroll 이후 anchor exact match가 실패하면 `[SMART_NEXT] Anchor exact match failed; using continuation fallback` 로그 후 `findAnchorContinuationCandidateIndex(...)`로 `startIndex=0` 전체를 재탐색하며, 우선순위를 `(1) pre-scroll anchor 하단의 trailing continuation 후보 > (2) scroll 후 하단 신규 노출 콘텐츠 > (3) 기타 unvisited visible 후보` 순으로 적용합니다. 상단 resurfaced 브랜딩/anchor 항목(예: `SmartThings`)은 continuation 후보에서 제외 또는 최하위로 강등합니다
