...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.4.0`: `CLICK_FOCUSED`에서 focused 노드 subtree가 비어(`childCount=0`) descendant metadata가 비는 경우, root raw tree에서 mirror node를 해석해 mirror 기준 descendant/actionable metadata 탐색을 선행한 뒤 기존 ancestor/root retarget 순서를 유지합니다. 디버그 로그(`click_focused_mirror_*`)와 결과 payload의 additive 필드(`mirrorResourceId/mirrorClassName/mirrorBounds`)를 추가해 관측성을 강화했습니다.
