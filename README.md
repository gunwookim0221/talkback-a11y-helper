...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.30.6`
  ...
  - 스크롤 히스토리는 `visitedHistory`(실제 포커스 성공 이력)와 `visibleHistory`(스크롤 직전 가시 이력)로 분리해 관리하며, continuation fallback에서는 `visibleHistory=true`라도 `visitedHistory=false`이면 새 컨텐츠 후보로 허용합니다
