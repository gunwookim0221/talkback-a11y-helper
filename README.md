...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.19.0`
  ...
  - `performSmartNext`가 다음 후보를 하단바로 판단해도, 하단바 진입 전에 `findIntermediateContentCandidateBeforeBottomBar(...)`로 마지막 본문 후보(특히 bottom bar 경계 0~60px 위에서 끝나는 얇은 trailing content)를 먼저 소진
  ...
  - `ACTION_ACCESSIBILITY_FOCUS` 직전에는 `ACTION_FOCUS`(입력 포커스)와 이중 `ACTION_CLEAR_ACCESSIBILITY_FOCUS`를 함께 시도해 시스템 거부 반응을 줄이고, 성공 판정은 실제 시스템 포커스 중심점이 타겟 중심점에서 10px 이내일 때만 유지합니다. 10px를 넘겨 벗어난 snap-back은 더 이상 성공 강제하지 않고 실패(`snap_back`)로 처리합니다
  - snap-back이 발생하면 더 이상 `moved`를 강제하지 않고 `snap_back` 실패로 처리하며, 실제 시스템 포커스 기준으로 `lastRequestedFocusIndex`를 재동기화해 다음 Smart Next의 루프를 완화
