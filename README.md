...
- `A11yNavigator`
  ...
  - `A11yFocusExecutor.VERSION = 1.4.0`: 수동 `ACTION_CLEAR_ACCESSIBILITY_FOCUS` 해제를 비활성화(`clearAccessibilityFocus(...)`는 no-op `true` 반환)해 Android Auto-Snap(포커스 최상단 강제 리셋) 경로를 차단했습니다. 또한 `requestAccessibilityFocusWithRetry(...)`를 단순 타겟 직접 포커스(`target.performAction(ACTION_ACCESSIBILITY_FOCUS)`) + 재시도 검증 흐름으로 정리해 복잡한 스나이퍼 분기를 제거했습니다.
