...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.13.2`
  ...
  - `ACTION_ACCESSIBILITY_FOCUS` 직전에는 `ACTION_FOCUS`(입력 포커스)와 이중 `ACTION_CLEAR_ACCESSIBILITY_FOCUS`를 함께 시도해 시스템 거부 반응을 줄이고, 성공 반환 뒤 실제 시스템 포커스 좌표가 타겟과 다르면 snap-back으로 판정하되 내부적으로는 해당 이동을 성공 처리해 한 칸씩만 전진
