...
- `A11yNavigator`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.4.6`: `shouldExcludeAsEmptyShell(...)`에서 `interactiveDescendantCount == 1`일 때 부모 컨테이너를 강제 제외하던 롤백 로직을 제거해, 자식 직접 포커스가 silent reject 되는 케이스를 방지하고 부모 노드가 정상적으로 traversal 트리에 남도록 복원했습니다.
