...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.13.0`
  ...
  - `ACTION_ACCESSIBILITY_FOCUS` 직전에는 `ACTION_FOCUS`(입력 포커스)와 이중 `ACTION_CLEAR_ACCESSIBILITY_FOCUS`를 함께 시도해 시스템 거부 반응을 줄이고, 성공 반환 뒤에도 실제 시스템 포커스 좌표가 타겟과 다르면 snap-back으로 판정해 즉시 다음 인덱스로 우회
  - snap-back이 발생해도 `lastRequestedFocusIndex`/`A11yStateStore.lastRequestedFocusIndex`에는 이미 시도한 인덱스를 유지해 다음 move 호출이 되돌아가지 않고 다음 후보부터 이어서 탐색
