...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.1.0`: `CLICK_FOCUSED` 실행 순서를 `direct → descendant(raw subtree BFS) → ancestor`로 일반화하고, 결과 payload에 `reason/path/attemptedResourceId/attemptedClassName/focused/target` 메타를 additive로 확장했습니다. 기존 action 이름과 브로드캐스트 계약은 유지됩니다.
