...
- `A11yNavigator`
  ...
  - `A11yFocusExecutor.VERSION = 1.4.6`: single-target 모드는 유지한 채, pre-focus readable alignment scroll 발동을 bottom-clipped/실제 가시성 부족 케이스로 제한해 일반 visible content candidate(예: Air Care)에서 조기 scroll이 선행되지 않도록 조정했습니다. 또한 authoritative commit 직후 top chrome/header 재부상 노이즈 억제를 scroll 액션 전용이 아닌 공통 경로로 적용해 QR code→우리 집 같은 regular move에서도 committed target UX 안정성을 높였습니다.
