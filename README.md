...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.7.8`
  ...
  - 노드 포커스 직전 `bounds.bottom > effectiveBottom`이면 하단 탭에 가려진(clipping) 상태로 판단하고 `ACTION_SHOW_ON_SCREEN`을 선호출해 온전한 사각형 노출을 유도
  - 스크롤 직전 화면에 보인 라벨을 `visibleHistory`로 저장하고, 스크롤 후 동일 라벨은 무조건 스킵해 중복 포커스/역주행을 차단
  - 스크롤 후 대기 시간을 `700ms + 100ms`로 단축하고 `[SMART_DEBUG] Index, Label, Y_Bottom, Eff_Bottom, InHistory` 좌표 기반 로그를 모든 후보 노드에 출력
