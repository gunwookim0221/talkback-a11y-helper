...
- `A11yNavigator`
  ...
  - `A11yFocusExecutor.VERSION = 1.3.5`: `requestAccessibilityFocusWithRetry(...)`가 `performAction` 반환값(`focused`)에 즉시 의존하지 않고, 매 시도마다 `ACTION_ACCESSIBILITY_FOCUS` 실행 → `150ms` 대기 → 실제 `findFocus`/`isAccessibilityFocused` 검증 순서로 성공 여부를 판정하도록 수정했습니다. 또한 `resolveFocusRetargetDecision(...)`의 성공 조건에서 `!isScrollAction -> true` 예외를 제거해 실제 포커스 불일치를 실패로 처리합니다.
