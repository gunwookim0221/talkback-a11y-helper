...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.68.0`
  - `A11yHelperService.VERSION = 1.4.9`: `CLICK_FOCUSED`의 local raw search를 traversal/candidate 판정으로 분리해, root raw tree child enumeration(BFS)에서 parent-child enqueue를 항상 수행하도록 정리했습니다. `click_focused_local_raw_expand`, `click_focused_local_raw_child`, `click_focused_local_raw_toolbar_summary` 로그를 추가해 toolbar branch(`setting_button_layout`, `settings_image`, `settings_new_badge`) 관측 여부를 추적하고, `click_focused_local_raw_candidate_skip` 사유 로그로 필터링 이전/이후 단계를 분리했습니다. giant/top/locality/global fallback 정책은 그대로 유지합니다.
  - `A11yPostScrollScanner.VERSION = 1.3.0`: regular move는 `singleTargetOnly` 모드로 진입해 `nextIndex` 하나만 시도하고 즉시 종료합니다. 즉 실패 시 같은 `move_smart` 호출 안에서 later candidate sweep(Add/More options/Food...)을 금지하고, multi-candidate scan은 post-scroll continuation 경로에서만 유지합니다.
  - `A11yFocusExecutor.VERSION = 1.4.8`: alias group 대표 후보에 포커스 성공 시 해당 그룹의 멤버 시그니처를 visited history에 함께 기록해, 다음 move에서 동일 logical duplicate가 재선택되지 않도록 조정했습니다(단일 타깃 모드 유지).
  - `A11yTraversalAnalyzer.VERSION = 1.10.0`: TalkBack-like 대표 노드의 actionable descendant 선별을 `clickable + enabled + visible + valid bounds` 기반으로 정교화하고, label/button-like class/대표 bounds 포함 여부를 가중치로 사용해 첫 child가 아닌 실제 activation에 가까운 하위 노드를 metadata로 보존합니다(기존 필드 유지, additive 확장).
