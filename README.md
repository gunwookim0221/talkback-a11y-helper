...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.75.9`
  ...
  - `A11yHelperService.VERSION = 1.5.6`: `moveFocusSmart` 예외 경로를 포함한 `[SMART_NEXT][final]` 로그를 계속 `Log.i(A11Y_HELPER, ...)` 경로로 유지해 실패 케이스에서도 기존 logcat 수집 필터에서 빠지지 않도록 보정했습니다(동작 로직 변경 없음).
  - `A11yCommandReceiver.VERSION = 1.2.4`: `service_unavailable`/`async_exception` 실패 경로의 `[SMART_NEXT][final]` 로그 레벨을 `Log.i(A11Y_HELPER, ...)`로 통일해 브로드캐스트 수신기 실패 종료도 동일한 logcat 경로로 노출되도록 맞췄습니다(동작 로직 변경 없음).
  ...
  - `A11yFocusExecutor.VERSION = 1.5.3`: `SMART_NEXT` focus commit 로그(`[SMART_NEXT][focus_commit]`)를 최종 판단 지점/실패 조기 반환 지점에서 `Log.i(A11Y_HELPER, ...)`로 통일해 실제 수집 중인 A11Y_HELPER INFO 경로와 맞췄습니다(동작 로직 변경 없음).
  - `A11yHelperService.VERSION = 1.5.8`: `dumpTree`의 실제 런타임 출력 지점(`DUMP_TREE_PART`/`DUMP_TREE_RESULT`) 인접에 `[SMART_NEXT][canary] stage='before_dump_tree'`, `[SMART_NEXT][canary] stage='after_dump_tree'`를 `Log.i("A11Y_HELPER", ...)`로 추가해 clean logcat에서 canary 도달 여부를 즉시 확인할 수 있게 했습니다(동작 로직 변경 없음).
  - `A11yTraversalAnalyzer.VERSION = 1.11.1`: 실제 수집 확인 로그(`[SMART_NEXT][static_text_included]`, `[SMART_NEXT][hero] rejected`) 인접에 `[SMART_NEXT][canary] stage='before_response'`, `[SMART_NEXT][canary] stage='before_finalize'`를 `Log.i("A11Y_HELPER", ...)`로 추가해 runtime 경로 추적성을 강화했습니다(동작 로직 변경 없음).
  - `A11yCommandReceiver.VERSION = 1.2.7`: 액션 분기 지점에서 실제 브로드캐스트 경로를 명확히 추적할 수 있도록 `ACTION_SMART_NEXT`에는 `[SMART_NEXT_ACTION][entry]`, `ACTION_DUMP_TREE`에는 `[DUMP_TREE_ACTION][entry]` 로그를 각각 추가했습니다(동작 로직 변경 없음).
  - `A11yCommandReceiver.VERSION = 1.2.8` / `A11yHelperService.VERSION = 1.5.11` / `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION = 2.75.4`: 원인 확정용 진단 빌드로 `[SMART_NEXT_DIAG]` req_id end-to-end 로그(수신/서비스 엔트리/후보 선택/global nav 판정/focus commit/final/exception)를 추가했습니다.
  - `A11yHelperService.VERSION = 1.5.9`: `moveFocusSmart`/`dumpTree` 진입 경로에 `[SMART_NEXT_ACTION][service|before_navigator|after_navigator]`, `[DUMP_TREE_ACTION][before_dump]` 로그를 추가해 SMART_NEXT와 DUMP_TREE의 실제 실행 경로를 액션 기준으로 분리 추적할 수 있게 했습니다(동작 로직 변경 없음).
  ...
  - `performSmartNext` 시작 시 현재 accessibility focus가 `android.webkit.WebView` top-level container에 고정되고 child가 존재하면, traversal 진입 전에 WebView subtree에서 `visible + (focusable/screenReaderFocusable) + labeled + valid bounds` 후보를 top/left 우선으로 1개만 선택해 즉시 포커스합니다. 성공 시 `status=moved`, `detail=webview_descend`를 반환하고 `[WEBVIEW_DESCEND] detected/candidate_count/selected_bounds/selected_label` 로그를 남깁니다. 후보가 없거나 포커스 실패면 기존 SMART_NEXT 경로로 그대로 fallback합니다.
...
- Python 클라이언트 알고리즘 버전: `CLIENT_ALGORITHM_VERSION = 1.7.68`
- 발화 조회 API
