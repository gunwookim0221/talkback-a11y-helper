...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.70.1`
  ...
  - `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION = 2.70.1`: OneConnect Settings generic row grouping의 title 판정에 `com.samsung.android.oneconnect:id/pref_title`(OneConnect 패키지 한정)를 안전 확장해 `Ask to add new devices` 같은 row가 container 1회 소비 뒤 title-only 후보로 재등장하지 않도록 same-row skip을 보강했습니다. representative는 계속 row container로만 유지하고 wrapperLayout/ScrollView/상위 컨테이너 제외 규칙은 그대로 유지합니다.
