...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.61.1`
  - `A11yPostScrollScanner.VERSION = 1.2.1`: `tryFocusCandidate(...)`에서 상단바 스킵을 `isScrollAction=true`일 때만 적용(`shouldSkipTopBar = isTopBar && request.isScrollAction`)해, 일반 순차 이동에서는 Top Bar 항목(예: 우리 집/Add/More options)을 정상 포커스하도록 수정
  - `A11yFocusExecutor.VERSION = 1.3.6`: `requestAccessibilityFocusWithRetry(...)` 루프에서 `ACTION_ACCESSIBILITY_FOCUS` 실행 직전에 `target.refresh()`를 복구해 stale node로 인한 silent reject를 줄이고, 이후 `150ms` 대기 + `target.refresh()` + 실제 `findFocus`/`isAccessibilityFocused` 검증 순서로 성공 여부를 판정합니다. 또한 `resolveFocusRetargetDecision(...)`의 성공 조건에서 `!isScrollAction -> true` 예외를 제거해 실제 포커스 불일치를 실패로 처리합니다.
  - `A11yTraversalAnalyzer.VERSION = 1.4.9`: `collectFocusableNodes(...)`에서 텍스트/설명이 상위 컨테이너로 흡수(`childAbsorbed=true`)되더라도 자식이 클릭 가능 또는 포커스 가능(`isClickable || isFocusable`)이면 재귀 탐색을 유지하도록 수정해, 상호작용 가능한 자식 누락으로 인한 Silent Reject를 방지했습니다.
