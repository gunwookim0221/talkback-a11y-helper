...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.3.0`: `CLICK_FOCUSED`에서 direct/descendant 실패 시 representative 노드의 actionable descendant 힌트(`actionableDescendant*`)를 우선 사용해 root tree 매칭을 시도하고, root retarget의 `local_band`는 상단 소형 타깃에서 inside/overlap 후보가 없으면 실패 허용으로 제한하여 대형 카드 오탭을 방지합니다. 결과 payload(`reason/path/attemptedResourceId/attemptedClassName/focused/target`)와 기존 브로드캐스트 action 이름은 유지됩니다.
