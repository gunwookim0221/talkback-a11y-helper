...
- `A11yNavigator`
  ...
  - `A11yFocusExecutor.VERSION = 1.4.5`: single-target 모드는 유지한 채, bottom-clipped 후보에서 pre-focus 정렬 후 bounds가 이동해도 intended candidate identity(패키지/클래스/윈도우/라벨·viewId·구조 안정성) 기반으로 verify 성공을 판정하도록 보강했습니다. 또한 pre-focus 정렬은 단순 `ACTION_SHOW_ON_SCREEN` 1회에 그치지 않고 readable content area 정렬(추가 scroll 포함)을 우선 목표로 강화했으며, 정렬이 실제 수행된 성공 케이스는 `moved_aligned` 상태로 구분합니다.
