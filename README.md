...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.8.1`
  ...
  - 노드 포커스 직전 `bounds.bottom > (effectiveBottom - 10)`이면 하단 탭에 가려진(clipping) 상태로 판단하고 API 26+에서는 `AccessibilityNodeInfo.AccessibilityAction.ACTION_SHOW_ON_SCREEN.id`를 호출하고, 미만 버전은 로그를 남긴 뒤 포커스 기반 스크롤 유도 로직을 유지
  - 스크롤 직전 화면에 보인 라벨을 `visibleHistory`로 저장하고, 스크롤 후 동일 라벨은 무조건 스킵해 중복 포커스/역주행을 차단
  - 스크롤 이후 대기 시간을 `700ms + 300ms`로 조정해 UI 갱신 안정성을 확보
  ...
  - 새 후보 노드에 `ACTION_ACCESSIBILITY_FOCUS`를 수행하기 직전에 50ms settling 지연을 추가해 이전 항목 공지 잔여 이벤트를 소거
  - v2.7.5에서 도입한 “제외 노드 포커스 강제 해제”와 “50ms settling 지연” 로직은 유지되며, 루프 폴백 시에는 필터를 초기화해 실제 첫 번째 콘텐츠 탐색을 우선
  ...
  - 포커스 성공 직후 노드 Top이 화면 상단 20% 기준보다 아래면 `ACTION_SHOW_ON_SCREEN`을 추가 호출해 상단 정렬을 유도
