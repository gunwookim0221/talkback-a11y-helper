...
- `A11yNavigator`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.8.2`: wrapper duplicate alias 판정을 동일 control의 `outer wrapper + inner actual action button` 구조까지 확장했습니다. 동일 package/유사 label/강한 bounds 중첩/ancestor 체인을 기본 조건으로 유지하면서, 카드 본체처럼 면적 비율이 큰 컨테이너(기본 3.2x 초과)는 제외해 카드 vs 내부 버튼 경계를 보존합니다. 또한 alias 대표 선택 점수에 `ImageButton/Switch/Toggle` 우선 가중치를 추가해 실제 포커스 확정이 일어나는 inner control이 대표로 선택되도록 보강했습니다. 추가로 collect/dedupe/empty-shell/alias/final normalize 단계별 추적 로그(`[TRACE_*]`)와 bounds+role 기반 dedupe를 도입해 parent row와 trailing action control의 분리를 강화했습니다.
