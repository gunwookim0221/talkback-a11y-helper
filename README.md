...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.67.0`
  - `A11yHelperService.VERSION = 1.0.3`: `TOUCH_BOUNDS_CENTER` 제스처 콜백 대기 중 메인 루퍼 블로킹으로 인한 timeout 구조를 줄이기 위해 callback handler를 전용 `HandlerThread`로 분리했고, callback 결과 공유를 `AtomicReference`로 정리했습니다. 기존 reason 분기(`completed/cancelled/dispatch false/timeout`)와 delay(100ms)·duration(90ms)·timeout(2800ms)은 유지합니다.
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.8.0`: wrapper duplicate alias 판정을 동일 control의 `outer wrapper + inner actual action button` 구조까지 확장했습니다. 동일 package/유사 label/강한 bounds 중첩/ancestor 체인을 기본 조건으로 유지하면서, 카드 본체처럼 면적 비율이 큰 컨테이너(기본 3.2x 초과)는 제외해 카드 vs 내부 버튼 경계를 보존합니다. 또한 alias 대표 선택 점수에 `ImageButton/Switch/Toggle` 우선 가중치를 추가해 실제 포커스 확정이 일어나는 inner control이 대표로 선택되도록 보강했습니다.
...
- Python 클라이언트 알고리즘 버전: `CLIENT_ALGORITHM_VERSION = 1.7.13`
- 발화 조회 API
