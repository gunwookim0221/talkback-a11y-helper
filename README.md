...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.11.2`
  ...
  - 동일 좌표·`<no-label>` 노드는 `findAndFocusFirstContent(...)`에서 즉시 건너뛰어 다음 인덱스로 점프하며, 같은 행(Y 좌표 동일)의 노드로 이동할 때는 포커스 명령 직전 50ms 지연을 추가
