...
- `A11yNavigator`
  ...
  - `A11yFocusExecutor.VERSION = 1.4.2`: single-target 모드는 유지하되 `requestAccessibilityFocusWithRetry(...)`/후속 안정화 검증을 짧은 polling window(50~100ms 간격, 400~700ms 총 창)로 보강해 `performAction` 직후 stale read로 인한 false negative를 줄이고, 최종 성공 판정은 실제 포커스 도달 여부로만 확정합니다.
