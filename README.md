...
- `A11yNavigator`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.10.1`: SmartThings OneConnect 상세 화면의 hero summary(상단 요약) 컨테이너가 `focusable=true`/`clickable=false` 형태일 때, 기존 대형 컨테이너 제외 규칙으로 누락되지 않도록 OneConnect 한정 조건(의미 있는 descendant label, 상단 content 구역, top/bottom chrome 제외, 과도한 interactive descendant 제외)에서 traversal 후보로 보존합니다.
...
- Python 클라이언트 알고리즘 버전: `CLIENT_ALGORITHM_VERSION = 1.7.33`
- 발화 조회 API
