...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.5.7`
  ...
  - `SMART_NEXT`에서 루트 BFS 기반 스크롤 후보 탐색이 실패하면 현재 포커스 노드 기준 부모 방향으로 올라가며 `isScrollable && ACTION_SCROLL_FORWARD`를 동시에 만족하는 스크롤 가능한 조상을 재탐색
  - 스크롤 후 첫 컨텐츠 재포커스 시 직전 포커스 노드와 `viewId` 또는 `text`가 같은 노드는 건너뛰어 중복 포커스를 방지
