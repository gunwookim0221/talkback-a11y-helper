...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.2.0`: `CLICK_FOCUSED` 실행 순서를 `direct → descendant(raw subtree BFS) → ancestor → root_retarget`으로 확장하고, focused node의 bounds/resourceId/content-desc/class/text를 단서로 root raw tree의 clickable 후보를 재탐색해 최적 대상을 선택합니다. 결과 payload(`reason/path/attemptedResourceId/attemptedClassName/focused/target`)와 기존 브로드캐스트 action 이름은 유지됩니다.
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.9.0`: TalkBack-like 대표 노드에 descendant actionable 메타(`hasClickableDescendant`, `hasFocusableDescendant`, `effectiveClickable`, `actionableDescendant*`)를 보존해 `focusable parent + clickable child` 구조에서도 하위 actionable 의미가 flat dump에서 소실되지 않도록 개선했습니다(기존 필드 유지, additive 확장).
