...
- `A11yNavigator`
  ...
  - `A11yFocusExecutor.VERSION = 1.3.7`: `requestAccessibilityFocusWithRetry(...)`를 레이스 컨디션 대응 로직으로 교체해 기본 재시도(`4회`)와 대기(`250ms`)를 늘리고, 로컬 `isAccessibilityFocused` 판정을 제거했습니다. 포커스 검증은 `A11yStateStore.currentRoot` 기준의 실제 글로벌 `findFocus(FOCUS_ACCESSIBILITY)` 좌표 일치로만 판단하며, `[FOCUS_EXEC]` 로그로 시도/실포커스/최종 실패 원인을 추적합니다.
