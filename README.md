...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.61.0`
  ...
  - `A11yFocusExecutor.VERSION = 1.3.3`: `findScrollableNode(...)`에서 클래스명에 `horizontal` 또는 `viewpager`가 포함된 스크롤 노드는 후보에서 즉시 제외해, `PRE_SCROLL` 수직 스크롤 단계에서 탭 바/뷰페이저가 스크롤을 가로채는 현상을 차단
