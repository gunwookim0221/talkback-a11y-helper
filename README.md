...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.7.9`
  ...
  - 스크롤 후 첫 콘텐츠 재포커스 시 직전 포커스 노드의 `contentDescription`이 현재 리스트에 존재하면 해당 노드 다음 인덱스부터 탐색을 시작해 이전 항목들을 물리적으로 건너뜀
  ...
  - 하단 고정 영역 판별: 클래스(`bottomnavigation/tablayout/navigationbar`) + 리소스 ID 키워드(`bottom/footer/tab_bar/navigation/menu_bar/menu_favorites/menu_devices/menu_/tab_/bottom_nav`)만 사용(좌표 기반 15% 임계값 제거)
  - 하단 경계의 리스트 카드(예: `Pet Care`)가 잘못 스킵되지 않도록 하단바 판별은 클래스/ID 키워드만 사용하고 좌표 기반(bottom 20%) 오인식 로직을 배제
  - 스크롤 직후 첫 유효 콘텐츠 포커스 시 `Thread.sleep(100)` 안정화 지연을 선반영하고, 성공 로그에 `[SMART_NEXT] Focused top-most content at Y=${bounds.top}`를 기록
