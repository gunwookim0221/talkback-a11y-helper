...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.68.7`
  ...
  - `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION = 2.68.7`: OneConnect Settings `Notifications` 행에서 `noti_title` + `notification_item_switch`가 동일 clickable/focusable row ancestor와 bounds 조건을 만족할 때만 좁게 alias group으로 묶고, 대표를 row container로 고정해 row 소비 이후 title/switch가 메인 traversal 후보로 재등장하지 않도록 조정했습니다.
