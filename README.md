...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.9.6`
  ...
  - 스크롤 직전 화면에 보인 라벨을 `visibleHistory`로 저장하되, 스크롤 후 히스토리 필터는 이미 본 `Fixed UI`를 상단 위치와 무관하게 우선 스킵하고, 고정되지 않은 컨텐츠에만 화면 상단 25% 예외를 적용합니다
  - 스크롤 후 동일 라벨이라도 후보가 화면 상단 25% 이내(`bounds.top < screenTop + screenBottom / 4`)라면 `!isFixedUi` 조건에서만 히스토리를 무시하고 재포커스를 허용하며, `Fixed UI`는 히스토리에 있으면 재포커스를 금지
