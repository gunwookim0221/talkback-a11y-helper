...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.68.1`
  ...
  - `A11yTraversalAnalyzer.VERSION = 1.8.4`: collect 단계의 trailing action control 보강은 유지하면서, normalize alias/wrapper 병합에서 `large labeled composite row` + `trailing independent action control` 조합은 duplicate 그룹으로 묶지 않도록 조정했습니다. 이로써 parent row와 trailing switch가 모두 유지되고, 순서도 row → switch로 보존됩니다.
