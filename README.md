...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.4.7`: `CLICK_FOCUSED`의 local raw search를 root raw tree full traversal로 고정해, resolved raw focus의 subtree childCount가 0이어도 전체 raw 노드를 순회하면서 top-right small actionable child(예: `settings_image`)를 수집하도록 수정했습니다. `click_focused_local_raw_scan_node`, `click_focused_local_raw_candidate_seen`, stage=`local_raw_collected` 로그를 통해 scan/collection/pipeline 진입을 분리 추적하며 giant/top/locality/global fallback 정책은 그대로 유지합니다.
