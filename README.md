...
- `A11yNavigator`
  ...
  - `A11yHelperService.VERSION = 1.4.5`: `CLICK_FOCUSED`에서 focused snapshot이 wrapper-only(`children=[]`)인 경우에도 root 트리에서 raw focused node를 재해석(`resourceId/class/bounds/focus 상태`)한 뒤 raw descendant BFS를 수행해 실제 clickable child(예: `settings_image`)를 mirror candidate source로 수집합니다. 수집 로그에 `source='raw_descendant'`, stage=`raw_descendant_collected`, raw resolve 결과(`[click_focused_raw_focus_resolve]`)를 추가했고 giant/top/locality/global fallback 정책은 유지합니다.
