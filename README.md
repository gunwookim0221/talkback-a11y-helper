...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.69.0`
  ...
  - `A11yNavigator.NAVIGATOR_ALGORITHM_VERSION = 2.69.0`: OneConnect Settings 문제 구간에서만 DEBUG 계측을 추가해 `decideInitialNextTarget(...)`의 normalize/collect 인덱스, duplicate/coordinate skip 전후 후보, 주변 window 후보 목록, Notifications same-row reselection 판정 경로, visited representative/member 기록을 추적할 수 있도록 했습니다(동작 로직 변경 없음).
