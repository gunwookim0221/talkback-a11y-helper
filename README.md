...
- `A11yNavigator`
  - 알고리즘 버전: `NAVIGATOR_ALGORITHM_VERSION = 2.31.7`
  ...
  - 스크롤 히스토리는 `visitedHistory`(실제 포커스 성공 이력)와 `visibleHistory`(스크롤 직전 가시 이력)로 분리해 관리하며, continuation fallback에서는 단순 `visitedHistory=false`만으로 채택하지 않고 pre-scroll anchor 하단 연속성(예: trailing continuation 카드)과 하단 신규 노출 여부를 우선 평가합니다
  ...
  - scroll 이후 anchor exact match가 실패하면 `[SMART_NEXT] Anchor exact match failed; using continuation fallback` 로그 후 `findAnchorContinuationCandidateIndex(...)`로 `startIndex=0` 전체를 재탐색하며, 우선순위를 `(1) pre-scroll focused anchor의 logical successor(예: item_knox_matrix -> item_privacy_notice) > (2) pre-scroll anchor 하단의 trailing continuation 후보 > (3) scroll 후 하단 신규 노출 콘텐츠 > (4) 기타 unvisited visible 후보` 순으로 적용합니다. 상단 resurfaced 브랜딩/anchor 항목(예: `SmartThings`, `History`)은 continuation 후보에서 제외 또는 최하위로 강등합니다
