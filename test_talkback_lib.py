...
class TouchIsinTest(unittest.TestCase):
    ...
    def test_check_talkback_ready_returns_enabled_but_not_ready_when_focus_payload_only(self):
        client = FakeA11yClient()
        client.enabled_services_payload = "foo:com.google.android.marvin.talkback/.TalkBackService"

        def fake_get_focus(*args, **kwargs):
            client.last_get_focus_trace["response_success"] = False
            return {"text": "Wi-Fi", "boundsInScreen": {"l": 0, "t": 0, "r": 10, "b": 10}}

        with patch.object(client, "check_helper_status", return_value=True), patch.object(client, "get_focus", side_effect=fake_get_focus):
            result = client.check_talkback_ready("SER")
    ...
    def test_check_talkback_ready_returns_enabled_when_talkback_and_helper_ready(self):
        ...
        def fake_get_focus(*args, **kwargs):
            client.last_get_focus_trace["response_success"] = True
            return {"text": "Wi-Fi", "boundsInScreen": {"l": 0, "t": 0, "r": 10, "b": 10}}
