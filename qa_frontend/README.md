...
- The frontend defaults Full Validation to the canonical Full scenario registry; source `enabled` flags remain targeted execution defaults and do not define Full membership.
...
- Selected Smoke and Selected Full both execute `script_test.py`; scenario enablement comes from the per-run runtime config generated by the backend. Full Validation uses the canonical registry set, while any partial selection is a Custom Run.
